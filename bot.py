...
def register_timed_next_step_handler(message, callback, *args, timeout=NEXT_STEP_TIMEOUT, **kwargs):
    ...
    bot.register_next_step_handler(message, _timed_next_step_wrapper, callback, args, kwargs, chat_id, user_id)
    key = (chat_id, user_id)
...
def _timed_next_step_wrapper(message, callback, args, kwargs, chat_id, expected_user_id):
    key = (chat_id, expected_user_id)
    with _next_step_lock:
        active = key in _next_step_expiries
    if not active:
        return
    # فقط خود صاحب مرحله اجازه ادامه دارد.
    if expected_user_id is not None and getattr(getattr(message, "from_user", None), "id", None) != expected_user_id:
        # پیام دیگران نباید مرحله کاربر را مصرف کند.
        register_timed_next_step_handler(message, callback, *args, **kwargs)
        return
    with _next_step_lock:
        _next_step_expiries.pop(key, None)
