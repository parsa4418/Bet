...
_AMOUNT_RE = re.compile(
    r"^(\d+(?:\.\d+)?)\s*(میلیون|میل|million|m|هزار|کا|ک|k)?$",
    re.IGNORECASE
)

# برای استفاده داخل رجکس‌های تشخیص پیام (تریگر دستورات متنی)؛ عدد + پسوند اختیاری کا/میل
AMOUNT_TOKEN = r"[\d۰-۹]+(?:[.,،][\d۰-۹]+)?\s*(?:میلیون|میل|million|m|هزار|کا|ک|k)?"
...
def extract_owner_id(call_data):
    """از callback_data به‌فرمت 'action|owner_id' شناسه صاحب پنل را استخراج می‌کند."""
    try:
...
def main_menu(call):
    ...
    caption = "به بات شرط‌بندی خوش اومدید🌹\nاز دکمه‌های زیر استفاده کنید:"
    # نکته امنیتی: پیام اصلی (که ممکنه پنل باز شخص دیگه‌ای باشه) دیگه پاک/ویرایش
    # نمیشه؛ همیشه یه پیام تازه فقط برای خودِ کلیک‌کننده فرستاده میشه، تا کسی
    # نتونه با زدن «منوی اصلی» پنل باز بقیه رو از بین ببره یا جایگزین کنه.
    try:
        ...
        bot.send_photo(
            ...
            photo=photo_url,
            caption=caption,
            reply_markup=main_menu_markup(clicker_id)
        )
    except Exception:
