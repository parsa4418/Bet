...
def process_group_message_for_diamond_hunt(message):
    ...
    و پیام‌های سرویس که Telegram به شکل message می‌فرستد هم از همین مسیر عبور می‌کنند.
    """
    ...
    with diamond_hunt_lock:
        ...
        # دقیقاً در هر ۲۰۰ پیام این گپ، شمارنده صفر می‌شود.
        _set_diamond_hunt(message.chat.id, message_count=0)

        # اگر الماس قبلی هنوز فعال است، این ۲۰۰ پیام ذخیره شده ولی
        # الماس جدید روی الماس قبلی قرار نمی‌گیرد.
        if row.get("active"):
            return
