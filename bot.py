...
FACTORY_WAREHOUSE_BASE_CAP = 5000
FACTORY_WAREHOUSE_CAP_PER_LEVEL = 5000          # ظرفیت(سطح) = پایه + (سطح-1) * این عدد
FACTORY_WAREHOUSE_UPGRADE_COST_PER_LEVEL = 15000  # هزینه(سطح) = سطح * این عدد

FACTORY_WORKERS_BASE_MAX = 1                     # حداکثر کارگر(سطح) = پایه + (سطح-1)
FACTORY_WORKERS_UPGRADE_COST_PER_LEVEL = 25000
FACTORY_WAGE_PER_WORKER_BASE = 1000              # دستمزد(سطح) = سطح * این عدد (روزانه، هر کارگر)

FACTORY_MACHINE_BASE_DRILL_SECONDS = 2.0
FACTORY_MACHINE_DRILL_REDUCTION_PER_LEVEL = 0.2
FACTORY_MACHINE_MIN_DRILL_SECONDS = 0.5
FACTORY_MACHINE_UPGRADE_COST_PER_LEVEL = 20000

FACTORY_XP_PER_HARVEST_UNIT = 500                # هر 500 الماس برداشت‌شده = 1 اکس‌پی
FACTORY_XP_NEEDED_PER_LEVEL = 500                # نیاز(سطح) = سطح * این عدد
...
def factory_warehouse_capacity(level):
    return FACTORY_WAREHOUSE_BASE_CAP + (level - 1) * FACTORY_WAREHOUSE_CAP_PER_LEVEL


def factory_warehouse_upgrade_cost(level):
    return level * FACTORY_WAREHOUSE_UPGRADE_COST_PER_LEVEL
...
def factory_workers_upgrade_cost(level):
    return level * FACTORY_WORKERS_UPGRADE_COST_PER_LEVEL


def factory_wage_per_worker(level):
    return level * FACTORY_WAGE_PER_WORKER_BASE


def factory_drill_seconds(level):
    return max(
        FACTORY_MACHINE_MIN_DRILL_SECONDS,
        FACTORY_MACHINE_BASE_DRILL_SECONDS - (level - 1) * FACTORY_MACHINE_DRILL_REDUCTION_PER_LEVEL
    )


def factory_machine_upgrade_cost(level):
    return level * FACTORY_MACHINE_UPGRADE_COST_PER_LEVEL
...
def factory_main_markup(owner_id):
    markup = types.InlineKeyboardMarkup()
    markup.add(types.InlineKeyboardButton("⛏ تولید", callback_data=f"fprod|{owner_id}"))
    markup.row(
...
def render_warehouse_panel(call, owner_id, factory):
    ...
    text = (
        ...
        f"🔺 ظرفیت بعد از ارتقا: {factory_warehouse_capacity(level + 1):,} الماس"
    )
    markup = types.InlineKeyboardMarkup()
...
def render_workers_panel(call, owner_id, factory):
    ...
    text = (
        ...
        f"⬆️ هزینه ارتقای سطح (سطح {level + 1}): {upgrade_cost:,} 💎\n"
    ...
    markup.add(types.InlineKeyboardButton(f"⬆️ ارتقای سطح ({upgrade_cost:,} 💎)", callback_data=f"fwkup|{owner_id}"))
    markup.add(types.InlineKeyboardButton("🔙 بازگشت", callback_data=f"fback|{owner_id}"))
    safe_edit_message(text, call.message.chat.id, call.message.message_id, reply_markup=markup)
...
def factory_hire_worker(call):
    ...
    if factory["workers_hired"] >= max_workers:
        ...
        return
    new_hired = factory["workers_hired"] + 1
    try:
