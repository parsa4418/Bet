    ...
    if prompt_id:
        ...
        if (not reply or reply.message_id != int(prompt_id) or
            ...
            try:
                ...
                pass
            # مرحله مصرف نشود؛ تایمر و state باقی می‌مانند.
            return True
...
ATOMIC_DB_MODE = os.environ.get("ATOMIC_DB_MODE", "on").strip().lower() == "on"
...
def add_ref_count(user_id):
    try:
        if ATOMIC_DB_MODE:
            return supabase.rpc("atomic_increment_ref_count", {
                "p_user_id": int(user_id), "p_delta": 1
            }).execute().data
        user = get_user(user_id)
        if user:
...
def register_word_diamond(user_id, chat_id, display_name):
...
def is_reply_to_panel(message, panel_message_id):
    """ورودی مرحله‌ای فقط وقتی معتبر است که کاربر روی همان پنل ربات ریپلای کرده باشد."""
    reply = getattr(message, "reply_to_message", None)
...
def cmd_start(message):
    ...
    if is_new:
        ...
        created = create_user(user_id, username, referred_by=referrer_id)
...
def text_referral(message):
    ...
    show_or_edit_panel(message.chat.id, user_id, "referral", f"👥 زیرمجموعه‌گیری\nتعداد زیرمجموعه‌های شما: {user.get('ref_count',0)}\nپاداش هر زیرمجموعه: {REFERRAL_BONUS:,} 💎\n\nلینک اختصاصی شما:\n{link}", back_to_main_menu_markup(user_id))
