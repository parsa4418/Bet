...
def _start_diamond_hunt(chat_id):
    try:
        msg = bot.send_message(
            ...
            reply_markup=_diamond_hunt_markup(1, 0)
        )
        _set_diamond_hunt(chat_id, active=True, hunt_message_id=msg.message_id, attempts=[])
    except Exception as e:
...
def process_group_message_for_diamond_hunt(message):
    ...
    with diamond_hunt_lock:
        ...
        if count < DIAMOND_HUNT_MESSAGE_INTERVAL:
            ...
            return
        # هر 200 پیام شمارنده ریست می‌شود؛ در هر گپ شمارنده کاملاً جداست.
...
def diamond_hunt_callback(call):
    ...
    with diamond_hunt_lock:
        row = _get_diamond_hunt(chat_id)
        if not row or not row.get("active") or int(row.get("hunt_message_id") or 0) != message_id:
            bot.answer_callback_query(call.id, "این الماس دیگر قابل برداشتن نیست ❌", show_alert=True)
            return
        attempts = row.get("attempts") or []
        expected_attempt = len(attempts) + 1
        if attempt_no != expected_attempt or attempt_no > 3:
            bot.answer_callback_query(call.id, "این تلاش قبلاً انجام شده یا معتبر نیست ❌", show_alert=True)
            return

        user_id = call.from_user.id
        if not get_user(user_id):
            bot.answer_callback_query(call.id, "اول /start را در پیوی بات بزن.", show_alert=True)
            return

        cost = DIAMOND_HUNT_COSTS[attempt_no - 1]
        if get_balance(user_id) < cost:
            bot.answer_callback_query(call.id, f"برای این تلاش {cost:,} 💎 لازم داری.", show_alert=True)
            return

        update_diamonds(user_id, -cost)
        name = _user_display_from_call(call)

        # شانس برد هر مرحله جداگانه است و با هر مرحله بیشتر می‌شود.
        ...
        if won:
            ...
            try:
                bot.answer_callback_query(call.id, "💍 الماس با موفقیت نجات پیدا کرد!", show_alert=False)
                safe_edit_message(
        ...
        bot.answer_callback_query(call.id, f"تلاش {attempt_no} ناموفق بود ❌")
        if attempt_no < 3:
            ...
            safe_edit_message(text, chat_id, message_id, reply_markup=_diamond_hunt_markup(attempt_no + 1, message_id))
        else:
            text = "❗ نتونستی الماس رو بگیری علت ⬇️\n\n" + _format_attempt_history(attempts)
            safe_edit_message(text, chat_id, message_id, reply_markup=None)
            _set_diamond_hunt(chat_id, active=False, hunt_message_id=None, attempts=[])
...
def webhook():
    if request.headers.get("content-type") == "application/json":
        ...
        # شمارش پیام‌ها قبل از اجرای هندلرها؛ بنابراین متن، عکس، استیکر، دایس و... هم شمرده می‌شوند.
        if getattr(update, "message", None) is not None:
            try:
                process_group_message_for_diamond_hunt(update.message)
            except Exception as e:
