...
def send_rtl(chat_id, text, *a, **k):
    return _original_send(chat_id, fix_text(text), *a, **k)
bot.send_message = send_rtl

if hasattr(bot, "edit_message_text"):
    _orig_edit = bot.edit_message_text
    def edit_rtl(text, *a, **k):
        return _orig_edit(fix_text(text), *a, **k)
    bot.edit_message_text = edit_rtl

if hasattr(bot, "send_photo"):
    _orig_photo = bot.send_photo
    def photo_rtl(chat_id, photo, *a, **k):
        if "caption" in k:
            k["caption"] = fix_text(k["caption"])
        return _orig_photo(chat_id, photo, *a, **k)
    bot.send_photo = photo_rtl


# ===== HARD BLOCK BOT MODE =====
@bot.callback_query_handler(func=lambda c: True)
def kill_bot_callbacks(c):
    if c.data and "bot" in c.data:
        try:
            bot.answer_callback_query(c.id, "⛔ بازی با ربات حذف شده", show_alert=True)
        except:
            pass
        return

@bot.message_handler(func=lambda m: True)
def kill_bot_messages(m):
    if m.text and "ربات" in m.text and "بازی" in m.text:
        try:
            bot.reply_to(m, "⛔ بازی با ربات حذف شده")
        except:
            pass
        return
