...
def display_name_with_tag(user_id, display_name):
    ...
    return f"{tag} {display_name}" if tag else display_name
...
def main_menu_markup(user_id=None):
    ...
    # Row 5: trophies + lootbox
    markup.row(
        types.InlineKeyboardButton("🏅 جام‌های من", callback_data=f"showtrophies|{user_id}" if user_id else "showtrophies"),
        types.InlineKeyboardButton("📦 جعبه شانس", callback_data=f"lootbox|{user_id}" if user_id else "lootbox")
    )
...
def cmd_account(message):
    ...
    markup.row(
        ...
        types.InlineKeyboardButton("🏅 جام‌های من", callback_data=f"showtrophies|{user_id}")
    )
    markup.row(
...
def handle_show_trophies(call):
    ...
    if not get_user(user_id):
        ...
        return
    markup = back_to_main_menu_markup(user_id)
    safe_edit_message(trophies_text_for_user(user_id), call.message.chat.id, call.message.message_id, reply_markup=markup)
...
def resolve_bet(bet_id, opponent_id, opponent_name, is_bot=False):
    ...
    winner_display = display_name_with_tag(winner_id, winner_name) if winner_id is not None else winner_name
    ...
    text = (
        ...
        f"🏆 برنده: {winner_display} (آیدی: {winner_id_str})\n"
...
def finalize_casino_game(msg_id):
    ...
    if score1 == score2:
        ...
        extra_line = f"💳 کسر بابت وام (۱۰٪): {loan_repay}\n" if loan_repay > 0 else ""
        winner_display = display_name_with_tag(winner_id, winner["name"]) if winner_id is not None else winner["name"]
        result_text = (
            f"{emoji} شرط تموم شد!\n"
            f"💎 مبلغ: {bet}\n"
            f"⚔️ {player1['name']} در برابر {player2['name']}\n\n"
            f"🏆 برنده: {winner_display} (امتیاز: {w_score})\n"
