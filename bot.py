...
# ================== مهلت ورود عدد/کد ==================
# هر مرحله‌ای که با register_next_step_handler منتظر ورودی کاربر می‌ماند
# حداکثر ۶۰ ثانیه فعال است. بعد از آن handler همان چت پاک می‌شود.
NEXT_STEP_TIMEOUT = 60
_next_step_expiries = {}
_next_step_lock = threading.Lock()

def register_timed_next_step_handler(message, callback, *args, timeout=NEXT_STEP_TIMEOUT, **kwargs):
    chat_id = getattr(getattr(message, "chat", None), "id", None)
    if chat_id is None:
        return

    # ثبت handler معمول TeleBot
    # توجه: کلید فقط بر اساس chat_id است، نه فرستنده‌ی پیامی که این هندلر را
    # ثبت کرده (که اغلب خودِ پیام ربات است، نه کاربر واقعی). تشخیص اینکه چه
    # کسی مجاز به ادامه‌دادن است، به‌عهده‌ی خودِ callback (پارامتر expected_user_id
    # که صریحاً به آن پاس داده می‌شود) است، نه این تابع.
    bot.register_next_step_handler(message, _timed_next_step_wrapper, callback, args, kwargs, chat_id)
    key = chat_id
    # پیام باتی که کاربر باید همان را بعد از پایان ۶۰ ثانیه ویرایش کند.
    ...
    with _next_step_lock:
        token = object()
        previous = _next_step_expiries.get(key)
        _next_step_expiries[key] = {"token": token, "prompt_message_id": prompt_message_id or (previous or {}).get("prompt_message_id")}

    def expire():
        with _next_step_lock:
            state = _next_step_expiries.get(key)
            if not state or state.get("token") is not token:
                return
            _next_step_expiries.pop(key, None)
        try:
...
def _timed_next_step_wrapper(message, callback, args, kwargs, chat_id):
    key = chat_id
    with _next_step_lock:
        active = key in _next_step_expiries
        if active:
            _next_step_expiries.pop(key, None)
    if not active:
        return
    # بررسی اینکه چه کسی مجاز به ادامه‌دادن این مرحله است، وظیفه‌ی خودِ callback
    # است (تقریباً همه‌ی callback ها پارامتر expected_user_id دارند و اگر پیام
    # از کاربر دیگری بود، خودشان دوباره register_timed_next_step_handler را
    # صدا می‌زنند تا منتظر پیام درست بمانند).
...
def bank_deposit_prompt(call):
    ...
    register_timed_next_step_handler(call.message, bank_deposit_step, user_id)
...
def bank_deposit_step(message, expected_user_id):
    if message.from_user.id != expected_user_id:
        ...
        register_timed_next_step_handler(message, bank_deposit_step, expected_user_id)
        return
    amount = parse_amount(message.text)
    if amount is None:
        msg = bot.reply_to(message, "❌ فرمت درست نیست. مثال: 500000 یا 500k یا 500کا یا 12.5میل")
        register_timed_next_step_handler(msg, bank_deposit_step, expected_user_id)
        return
    if amount <= 0:
...
def bank_withdraw_prompt(call):
    ...
    register_timed_next_step_handler(call.message, bank_withdraw_step, user_id)

def bank_withdraw_step(message, expected_user_id):
    if message.from_user.id != expected_user_id:
        # پیام از یه نفر دیگه بود؛ نادیده می‌گیریم ولی منتظر پیام خودِ کاربر می‌مونیم
        register_timed_next_step_handler(message, bank_withdraw_step, expected_user_id)
        return
    amount = parse_amount(message.text)
    if amount is None:
        msg = bot.reply_to(message, "❌ فرمت درست نیست. مثال: 500000 یا 500k یا 500کا یا 12.5میل")
        register_timed_next_step_handler(msg, bank_withdraw_step, expected_user_id)
        return
    if amount <= 0:
        msg = bot.reply_to(message, "❌ مبلغ باید بیشتر از صفر باشد.")
        register_timed_next_step_handler(msg, bank_withdraw_step, expected_user_id)
...
def loan_menu(call):
    ...
    register_timed_next_step_handler(call.message, loan_amount_step, user_id, remaining)

def loan_amount_step(message, expected_user_id, remaining):
    if message.from_user.id != expected_user_id:
        # پیام از یه نفر دیگه بود؛ نادیده می‌گیریم ولی منتظر پیام خودِ کاربر می‌مونیم
        register_timed_next_step_handler(message, loan_amount_step, expected_user_id, remaining)
        return
    amount = parse_amount(message.text)
    if amount is None:
        msg = bot.reply_to(message, "لطفاً یه مبلغ درست بفرست. مثال: 100000 یا 100k یا 100کا")
        register_timed_next_step_handler(msg, loan_amount_step, expected_user_id, remaining)
        return
...
def help_main_markup(user_id):
    ...
    markup.row(types.InlineKeyboardButton("👥 زیرمجموعه گیری", callback_data=f"help_topic|{user_id}|referral"))
    markup.row(types.InlineKeyboardButton("🏠 منوی اصلی", callback_data=f"mainmenu|{user_id}"))
    return markup
...
def handle_account_transfer_button(call):
    ...
    register_timed_next_step_handler(call.message, ask_transfer_target, owner_id)

def ask_transfer_target(message, owner_id):
    if message.from_user.id != owner_id:
        register_timed_next_step_handler(message, ask_transfer_target, owner_id)
        return
    ...
    if len(parts) != 2 or not parts[0].isdigit() or amount is None:
        ...
        register_timed_next_step_handler(msg, ask_transfer_target, owner_id)
        return
...
def mine_panel_entry(message):
    ...
    register_timed_next_step_handler(msg, mine_bet_step, message.from_user.id, msg.message_id)

def mine_bet_step(message, expected_user_id, prompt_msg_id):
    if message.from_user.id != expected_user_id:
        # پیام از یه نفر دیگه بود؛ نادیده می‌گیریم ولی منتظر پیام خودِ کاربر می‌مونیم
        register_timed_next_step_handler(message, mine_bet_step, expected_user_id, prompt_msg_id)
        return
    amount = parse_amount(message.text)
    if amount is None:
        msg = bot.reply_to(message, "❌ مبلغ نامعتبره. یه عدد درست بفرست (مثلاً 100000 یا 100k):")
        register_timed_next_step_handler(msg, mine_bet_step, expected_user_id, prompt_msg_id)
        return
    if get_balance(expected_user_id) < amount:
...
def factory_production_prompt(call):
    ...
    safe_edit_message(text, call.message.chat.id, call.message.message_id, reply_markup=None)
    register_timed_next_step_handler(call.message, factory_production_step, owner_id, call.message.message_id)

def factory_production_step(message, expected_user_id, panel_msg_id):
    if message.from_user.id != expected_user_id:
        register_timed_next_step_handler(message, factory_production_step, expected_user_id, panel_msg_id)
        return
    ...
    if amount is None or amount <= 0:
        ...
        register_timed_next_step_handler(msg, factory_production_step, expected_user_id, panel_msg_id)
        return
    if amount > stored:
...
def casino_custom_amount_prompt(call):
    ...
    register_timed_next_step_handler(call.message, casino_custom_amount_step, game_key, owner_id, call.message.message_id)

def casino_custom_amount_step(message, game_key, expected_user_id, panel_msg_id):
    if message.from_user.id != expected_user_id:
        # پیام از یه نفر دیگه بود؛ نادیده می‌گیریم ولی منتظر پیام خودِ کاربر می‌مونیم
        register_timed_next_step_handler(message, casino_custom_amount_step, game_key, expected_user_id, panel_msg_id)
        return

    amount = parse_amount(message.text)
    if amount is None:
        msg = bot.reply_to(message, "لطفاً یه مبلغ درست بفرست. مثال: 250 یا 250k")
        register_timed_next_step_handler(msg, casino_custom_amount_step, game_key, expected_user_id, panel_msg_id)
        return
...
def _jewelry_ready_text(rows):
    ...
    total = 0
    if len(rows) == 1:
        lines.append("💍 1 انگشتر با موفقیت ساخته شد ✅")
    else:
        lines.append(f"💍 {len(rows)} انگشتر با موفقیت ساخته شد ✅")
    lines.append("")
    for row in rows:
        gem = JEWELRY_GEMS.get(row.get("gem_key"), JEWELRY_GEMS["agate"])
        price = int(row.get("payout_total", 0) or 0)
        total += price
        lines.extend([
    ...
    lines.append(f"💎 مبلغ کل انگشترهای آماده: {total:,} الماس")
    return "\n".join(lines), total
...
def _jewelry_active_text(rows):
    if not rows:
        return None
    nearest = min(
        max(0, int((datetime.fromisoformat(str(r["finish_at"]).replace("Z", "+00:00")).replace(tzinfo=None) - _jewelry_now()).total_seconds()))
        for r in rows
    )
    return (
        "💍 ساخت انگشتر\n\n"
        f"تعداد انگشتر درحال ساخت {len(rows)} عدد 💍\n"
        f"زمان انتظار : {_format_jewelry_duration(nearest)} ⏳"
    )
...
def submit_vote_prompt(call):
    ...
    safe_edit_message(text, call.message.chat.id, call.message.message_id, reply_markup=markup)
    register_timed_next_step_handler(call.message, process_vote_code, tournament_id, voter_id, call.message.message_id)
...
def process_vote_code(message, tournament_id, voter_id, original_msg_id):
    ...
    if len(code) != 6:
        ...
        register_timed_next_step_handler(message, process_vote_code, tournament_id, voter_id, original_msg_id)
        return
    # ثبت رأی
...
def admin_start_tournament(call):
    ...
    safe_edit_message(text, call.message.chat.id, call.message.message_id, reply_markup=None)
    register_timed_next_step_handler(call.message, admin_set_prize_step, 1, {})  # step 1, prizes dict
...
def admin_set_prize_step(message, rank, prizes):
    ...
    if amount is None:
        ...
        register_timed_next_step_handler(msg, admin_set_prize_step, rank, prizes)
        return
    ...
    if rank < 10:
        ...
        bot.reply_to(message, text)
        register_timed_next_step_handler(message, admin_set_prize_step, next_rank, prizes)
    else:
...
def admin_broadcast(call):
    ...
    safe_edit_message(text, call.message.chat.id, call.message.message_id, reply_markup=None)
    register_timed_next_step_handler(call.message, admin_send_broadcast)
...
def admin_add_diamond_prompt(call):
    ...
    safe_edit_message(text, call.message.chat.id, call.message.message_id, reply_markup=None)
    register_timed_next_step_handler(call.message, admin_add_diamond_execute)
...
def admin_remove_diamond_prompt(call):
    ...
    text = "➖ لطفاً آیدی عددی کاربر و مقدار الماس را به صورت زیر وارد کنید:\n`<user_id> <amount>`\nمثال: `123456789 50`"
    safe_edit_message(text, call.message.chat.id, call.message.message_id, reply_markup=None)
    register_timed_next_step_handler(call.message, admin_remove_diamond_execute)
