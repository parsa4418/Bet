...
def webhook():
    ...
    if request.is_json:
        ...
        # پیام‌های معمولی را مستقیم به dispatcher پیام‌ها می‌دهیم.
        # این کار باعث می‌شود همه‌ی @bot.message_handler های متنی
        # (مثل موجودی، بانک، کازینو، گردونه و...) حتماً پردازش شوند.
        # برای update های غیرپیامی (مثل callback query) همان dispatcher اصلی استفاده می‌شود.
        if incoming_message is not None:
            bot.process_new_messages([incoming_message])
        else:
