...
# ================== سیستم دزدی ==================
THEFT_ITEM_PRICES = {"bag":None,"flashlight":None,"car":None,"lockpick":None,"metal_detector":None,"hacker_pc":None,"bomb":None,"laser_cutter":None}
THEFT_ITEMS = {
 "house":[("bag","👜 کیف"),("flashlight","🔦 چراغ قوه"),("car","🚗 ماشین"),("lockpick","🔓 دستگاه قفل باز کن"),("metal_detector","🔎 فلز یاب"),("hacker_pc","💻 کامپیوتر هکر")],
 "bank":[("bag","👜 کیف"),("flashlight","🔦 چراغ قوه"),("car","🚗 ماشین"),("metal_detector","🔎 فلز یاب"),("bomb","💣 بمب"),("laser_cutter","🔴 برش دهنده لیزری"),("hacker_pc","💻 کامپیوتر هکر")]
}
THEFT_ASSISTANTS=[("camera_hacker","📹 هکر دوربین"),("driver","🚗 راننده ماشین"),("safecracker","🔐 صندوق باز کن"),("locksmith","🔑 کلید ساز"),("bagman","👜 حمال کیف")]
THEFT_ASSISTANT_WAGES={k:None for k,_ in THEFT_ASSISTANTS}
...
_theft_sessions={}
_theft_target_pending={}
_theft_assistant_pending={}
_theft_lock=threading.RLock()

def _theft_inv(user_id):
    progress=get_progress(user_id); cosmetics=dict(progress.get("cosmetics") or {})
    inv=dict(cosmetics.get("theft_inventory") or {}); inv.setdefault("items",{})
    return cosmetics,inv

def get_theft_inventory(user_id):
    try:return _theft_inv(user_id)[1]
    except Exception:return {"items":{}}

def _save_theft_inv(user_id,inv,cosmetics=None):
    try:
        if cosmetics is None: cosmetics=_theft_inv(user_id)[0]
        cosmetics["theft_inventory"]=inv
        supabase.table("user_progress").update({"cosmetics":cosmetics}).eq("user_id",user_id).execute(); return True
    except Exception as e: logging.error(f"theft inventory save: {e}"); return False

def _theft_add_item(user_id,key):
    c,i=_theft_inv(user_id); b=dict(i.get("items") or {}); b[key]=int(b.get(key,0) or 0)+1; i["items"]=b; return _save_theft_inv(user_id,i,c)

def _theft_count(user_id,key): return int((get_theft_inventory(user_id).get("items") or {}).get(key,0) or 0)

def _theft_consume_items(user_id,keys):
    c,i=_theft_inv(user_id); b=dict(i.get("items") or {})
    if any(int(b.get(k,0) or 0)<=0 for k in keys): return False
    for k in keys:b[k]=int(b.get(k,0) or 0)-1
    i["items"]=b; return _save_theft_inv(user_id,i,c)

def _theft_item_name(key):
    for entries in THEFT_ITEMS.values():
        for k,n in entries:
            if k==key:return n
    return key
...
def _theft_member_ok(chat_id,user_id):
    try:return bot.get_chat_member(chat_id,int(user_id)).status in ("creator","administrator","member","restricted")
    except Exception as e: logging.warning(f"theft membership check: {e}"); return False

def theft_store_markup(user_id):
    m=types.InlineKeyboardMarkup(); m.row(colored_button("🧰 وسایل",callback_data=f"theftshop|items|{user_id}"),colored_button("🧑‍🤝‍🧑 دستیارها",callback_data=f"theftshop|assistants|{user_id}")); m.add(colored_button("🕵️ پنل دزدی",callback_data=f"theftrefresh|{user_id}")); return m

def theft_items_markup(user_id,mode="house"):
    m=types.InlineKeyboardMarkup(); inv=get_theft_inventory(user_id).get("items") or {}
    for k,n in THEFT_ITEMS[mode]:
        price=THEFT_ITEM_PRICES.get(k); ptxt=f"{price:,}" if price is not None else "بزودی"; m.add(colored_button(f"🛒 {n} | {ptxt} | موجودی {int(inv.get(k,0) or 0)}",callback_data=f"theftbuy|{k}|{user_id}"))
    m.add(colored_button("🔙 فروشگاه",callback_data=f"theftstore|{user_id}")); return m

def theft_assistants_markup(user_id):
    m=types.InlineKeyboardMarkup()
    for k,n in THEFT_ASSISTANTS:
        wage=THEFT_ASSISTANT_WAGES.get(k); w=f"{wage:,}" if wage is not None else "بزودی"; m.add(colored_button(f"🧑‍🤝‍🧑 {n} | سهم/دستمزد {w}",callback_data=f"theftnoop|{user_id}"))
    m.add(colored_button("🔙 فروشگاه",callback_data=f"theftstore|{user_id}")); return m

def theft_main_markup(user_id):
    m=types.InlineKeyboardMarkup(); m.row(colored_button("🏠 دزدی از خانه",callback_data=f"theftmode|house|{user_id}"),colored_button("🏦 دزدی از بانک",callback_data=f"theftmode|bank|{user_id}")); m.add(colored_button("🛒 فروشگاه",callback_data=f"theftstore|{user_id}")); return m

def theft_session_text(chat_id,user_id):
    with _theft_lock:s=dict(_theft_sessions.get((chat_id,user_id),{}))
    mode=s.get("mode","house"); items=s.get("items",[]); aids=s.get("assistants",[]); risk=max(THEFT_POLICE_MIN_CHANCE,THEFT_POLICE_BASE_CHANCE-THEFT_POLICE_REDUCTION_PER_ROLE*(len(items)+len(aids))); mx=THEFT_BANK_MAX_PERCENT if mode=="bank" else THEFT_HOUSE_MAX_PERCENT
    return ("◈ ━━━━━ 𝗧𝗵𝗲𝗳𝘁 ━━━━━ ◈\n"+f"☻ | نوع دزدی : {'🏠 خانه' if mode=='house' else '🏦 بانک'}\n\n"+f"🎯 | هدف : {s.get('target_name','انتخاب نشده')}\n"+f"🧰 | وسایل : {', '.join(_theft_item_name(k) for k in items) or 'ندارد'}\n"+f"🧑‍🤝‍🧑 | دستیارها : {', '.join(_theft_assistant_name(k) for k in aids) or 'ندارد'}\n\n"+f"💰 | حداکثر قابل دزدی : {mx}% موجودی هدف\n"+f"🚨 | احتمال دستگیری : {risk}%\n\n✓ | هرچه وسایل و دستیار بیشتری همراهت باشد، احتمال دستگیری کمتر می‌شود.\n✓ | سهم دستیارها به ادمین اول واریز می‌شود.")

def theft_session_markup(user_id,chat_id):
    with _theft_lock:s=_theft_sessions.get((chat_id,user_id),{})
    m=types.InlineKeyboardMarkup()
    if not s.get("target_id"):m.add(colored_button("🎯 انتخاب هدف",callback_data=f"thefttarget|{s.get('mode','house')}|{user_id}"))
    else:
        m.add(colored_button("➕ افزودن دستیار",callback_data=f"theftassistant|{user_id}")); m.add(colored_button("🧰 انتخاب وسایل",callback_data=f"theftgear|{user_id}"));
        if s.get("items"):m.add(colored_button("🚨 شروع دزدی",callback_data=f"theftstart|{user_id}"))
    m.add(colored_button("🛒 فروشگاه",callback_data=f"theftstore|{user_id}")); return m

def _theft_run(chat_id,user_id):
    key=(chat_id,user_id)
    with _theft_lock:s=dict(_theft_sessions.get(key,{}))
    mode=s.get("mode","house"); target=s.get("target_id"); items=list(s.get("items",[])); aids=list(s.get("assistants",[]))
    if not target:return False,"❌ ابتدا هدف دزدی را انتخاب کن."
    if target==user_id:return False,"❌ نمی‌تونی از خودت دزدی کنی."
    if not _theft_member_ok(chat_id,target):return False,"❌ هدف دیگر عضو همین گروه نیست."
    if any(a in (user_id,target) or not _theft_member_ok(chat_id,a) for a in aids):return False,"❌ یکی از دستیارها عضو همین گروه نیست."
    if not items:return False,"❌ حداقل یک وسیله لازم است."
    if not all(_theft_count(user_id,k)>0 for k in items):return False,"❌ یکی از وسایل انتخاب‌شده را نداری."
    if mode=="bank" and not get_bank_account_number(target):return False,"❌ هدف حساب بانکی فعالی ندارد."
    ...
    with _get_user_financial_lock(first):
      with _get_user_financial_lock(second):
        if mode=="bank":
            apply_bank_interest(target); balance=get_bank_balance(target)
        else: balance=int(get_balance(target) or 0)
        max_stolen=int(balance*mx/100)
        if max_stolen<=0:return False,"❌ موجودی قابل دزدی هدف صفر است."
        stolen=random.randint(1,max_stolen)
        risk=max(THEFT_POLICE_MIN_CHANCE,THEFT_POLICE_BASE_CHANCE-THEFT_POLICE_REDUCTION_PER_ROLE*(len(items)+len(aids)))
        if random.random()<risk/100:
            if not _theft_consume_items(user_id,items):return False,"❌ مصرف وسایل انجام نشد."
            with _theft_lock:_theft_sessions.pop(key,None)
            return True,f"🚔 عملیات لو رفت!\n\n🚨 | احتمال دستگیری : {risk}%\n❌ | هیچ الماسی دزدیده نشد.\n🔒 | وسایل عملیات مصرف شدند.\n⛓ | سیستم زندان بعداً اضافه می‌شود."
        if mode=="bank": ok=change_bank_balance(target,-stolen)
        else: ok=update_diamonds(target,-stolen) is not None
        if not ok:return False,"❌ برداشت از موجودی هدف انجام نشد."
        if not _theft_consume_items(user_id,items):
            if mode=="bank":change_bank_balance(target,stolen)
            else:update_diamonds(target,stolen)
            return False,"❌ مصرف وسایل انجام نشد؛ مبلغ برگشت داده شد."
        parts=1+len(aids); initiator=stolen//parts; initiator+=stolen-initiator*parts; assistant_total=stolen-initiator
        if update_diamonds(user_id,initiator) is None:
            if mode=="bank":change_bank_balance(target,stolen)
            else:update_diamonds(target,stolen)
            return False,"❌ واریز سهم شما انجام نشد؛ عملیات برگشت خورد."
        if assistant_total and update_diamonds(ADMIN_IDS[0],assistant_total) is None:
            update_diamonds(user_id,-initiator)
            if mode=="bank":change_bank_balance(target,stolen)
            else:update_diamonds(target,stolen)
            return False,"❌ سهم دستیارها به ادمین واریز نشد؛ عملیات برگشت خورد."
    with _theft_lock:_theft_sessions.pop(key,None)
    return True,("💰 دزدی با موفقیت انجام شد!\n\n"+f"🎯 | هدف : {s.get('target_name','کاربر')}\n"+f"💎 | مبلغ دزدیده‌شده : {stolen:,}\n"+f"👤 | سهم شما : {initiator:,} 💎\n"+f"🧑‍🤝‍🧑 | سهم دستیارها : {assistant_total:,} 💎\n"+f"🚨 | احتمال دستگیری : {risk}%\n🏦 | سهم دستیارها به ادمین اول واریز شد.")
...
def _theft_target_pending_handler(m): return m.chat.type in ("group","supergroup") and not getattr(m.from_user,"is_bot",False) and (m.chat.id,m.from_user.id) in _theft_target_pending
def _theft_assistant_pending_handler(m): return m.chat.type in ("group","supergroup") and not getattr(m.from_user,"is_bot",False) and (m.chat.id,m.from_user.id) in _theft_assistant_pending

@bot.message_handler(func=_theft_target_pending_handler)
def theft_target_reply(message):
    key=(message.chat.id,message.from_user.id); mode=_theft_target_pending.pop(key,None); r=getattr(message,"reply_to_message",None); u=getattr(r,"from_user",None) if r else None
    if not u or getattr(u,"is_bot",False) or u.id==message.from_user.id or not _theft_member_ok(message.chat.id,u.id): bot.reply_to(message,"❌ روی پیام یک عضو واقعی همین گروه ریپلای کن."); _theft_target_pending[key]=mode; return
    with _theft_lock:s=_theft_sessions.setdefault(key,{"mode":mode,"items":[],"assistants":[]}); s.update({"mode":mode,"target_id":u.id,"target_name":u.first_name or "کاربر"})
    show_or_edit_panel(message.chat.id,message.from_user.id,"theft",theft_session_text(*key),theft_session_markup(message.from_user.id,message.chat.id))

@bot.message_handler(func=_theft_assistant_pending_handler)
def theft_assistant_reply(message):
    key=(message.chat.id,message.from_user.id); _theft_assistant_pending.pop(key,None); r=getattr(message,"reply_to_message",None); u=getattr(r,"from_user",None) if r else None
    with _theft_lock:s=_theft_sessions.get(key)
    if not s or not s.get("target_id"):bot.reply_to(message,"❌ ابتدا هدف را انتخاب کن.");return
    if not u or getattr(u,"is_bot",False) or u.id in (message.from_user.id,s.get("target_id")) or not _theft_member_ok(message.chat.id,u.id):bot.reply_to(message,"❌ روی پیام یک عضو معتبر همین گروه ریپلای کن.");_theft_assistant_pending[key]=True;return
    with _theft_lock:
        if u.id in s.setdefault("assistants",[]):bot.reply_to(message,"❌ این شخص قبلاً دستیار شده.");return
        s["assistants"].append(u.id)
    show_or_edit_panel(message.chat.id,message.from_user.id,"theft",theft_session_text(*key),theft_session_markup(message.from_user.id,message.chat.id))

@bot.message_handler(func=lambda m:text_is(m,"دزدی") and not _has_pending_step(m))
def theft_entry(message):
    if message.chat.type not in ("group","supergroup"):bot.reply_to(message,"❌ بخش دزدی فقط داخل گروه فعال است.");return
    if not get_user(message.from_user.id):create_user(message.from_user.id,get_display_name(message.from_user))
    key=(message.chat.id,message.from_user.id)
    with _theft_lock:_theft_sessions[key]={"mode":"house","items":[],"assistants":[]}
    show_or_edit_panel(message.chat.id,message.from_user.id,"theft","◈ ━━━━━ 𝗧𝗵𝗲𝗳𝘁 ━━━━━ ◈\n\n✓ | نوع دزدی را انتخاب کنید:",theft_main_markup(message.from_user.id))

@bot.callback_query_handler(func=lambda c:c.data.startswith("theftmode|"))
def theft_mode(c):
    owner=check_panel_owner(c)
    if owner is None:return
    mode=c.data.split("|")[1];key=(c.message.chat.id,owner)
    with _theft_lock:_theft_sessions[key]={"mode":mode,"items":[],"assistants":[]};_theft_target_pending[key]=mode
    bot.answer_callback_query(c.id);safe_edit_message(("🏠 دزدی از خانه" if mode=="house" else "🏦 دزدی از بانک")+"\n\n✓ | روی پیام عضو هدف ریپلای کن و یک متن بفرست.",c.message.chat.id,c.message.message_id,None)

@bot.callback_query_handler(func=lambda c:c.data.startswith("thefttarget|"))
def theft_target_button(c):
    owner=check_panel_owner(c)
    if owner is None:return
    _theft_target_pending[(c.message.chat.id,owner)]=c.data.split("|")[1];bot.answer_callback_query(c.id,"روی پیام هدف ریپلای کن و یک متن بفرست.")

@bot.callback_query_handler(func=lambda c:c.data.startswith("theftassistant|"))
def theft_assistant_button(c):
    owner=check_panel_owner(c)
    if owner is None:return
    _theft_assistant_pending[(c.message.chat.id,owner)]=True;bot.answer_callback_query(c.id,"روی پیام دستیار ریپلای کن و یک متن بفرست.");safe_edit_message("🧑‍🤝‍🧑 | روی پیام دستیار ریپلای کن و یک متن بفرست.",c.message.chat.id,c.message.message_id,None)

@bot.callback_query_handler(func=lambda c:c.data.startswith("theftgear|"))
def theft_gear(c):
    owner=check_panel_owner(c)
    if owner is None:return
    with _theft_lock:s=_theft_sessions.get((c.message.chat.id,owner))
    if not s or not s.get("target_id"):bot.answer_callback_query(c.id,"ابتدا هدف را انتخاب کن.",show_alert=True);return
    m=types.InlineKeyboardMarkup();inv=get_theft_inventory(owner).get("items") or {}
    for k,n in THEFT_ITEMS[s.get("mode","house")]:m.add(colored_button(f"{'✓ ' if k in s.get('items',[]) else ''}{n} ({int(inv.get(k,0) or 0)})",callback_data=f"theftselect|{k}|{owner}"))
    m.add(colored_button("🔙 برگشت",callback_data=f"theftrefresh|{owner}"));bot.answer_callback_query(c.id);safe_edit_message("🧰 | وسایل عملیات را انتخاب کن.\nعدد داخل پرانتز = موجودی",c.message.chat.id,c.message.message_id,m)
...
def theft_select(c):
    ...
    with _theft_lock:s=_theft_sessions.get((c.message.chat.id,owner))
    if not s:return
    if k in s.setdefault("items",[]):s["items"].remove(k);msg="وسیله حذف شد."
    elif _theft_count(owner,k)<=0:msg="❌ این وسیله را نداری؛ اول از فروشگاه تهیه کن."
    else:s["items"].append(k);msg="وسیله اضافه شد."
    bot.answer_callback_query(c.id,msg);theft_gear(c)

@bot.callback_query_handler(func=lambda c:c.data.startswith("theftstart|"))
def theft_start(c):
    owner=check_panel_owner(c)
    if owner is None:return
    bot.answer_callback_query(c.id,"در حال انجام عملیات...");ok,text=_theft_run(c.message.chat.id,owner);safe_edit_message(text,c.message.chat.id,c.message.message_id,theft_main_markup(owner) if ok else theft_session_markup(owner,c.message.chat.id))

@bot.callback_query_handler(func=lambda c:c.data.startswith("theftrefresh|"))
def theft_refresh(c):
    owner=check_panel_owner(c)
    if owner is None:return
    bot.answer_callback_query(c.id);safe_edit_message(theft_session_text(c.message.chat.id,owner),c.message.chat.id,c.message.message_id,theft_session_markup(owner,c.message.chat.id))
...
def theft_store(c):
    ...
    if owner is None:return
    bot.answer_callback_query(c.id);safe_edit_message("🛒 فروشگاه دزدی\n\n✓ | دسته مد نظر را انتخاب کنید.",c.message.chat.id,c.message.message_id,theft_store_markup(owner))
...
def theft_shop(c):
    ...
    if cat=="items":
        ...
        safe_edit_message("🛒 وسایل دزدی\n\n✓ | دسته : "+("خانه" if mode=="house" else "بانک")+"\n\nقیمت‌ها فعلاً بزودی هستند.",c.message.chat.id,c.message.message_id,theft_items_markup(owner,mode))
    else:safe_edit_message("🧑‍🤝‍🧑 دستیارها\n\n✓ | دستیارها داخل عملیات انتخاب می‌شوند و سهمشان به ادمین اول می‌رسد.\nقیمت/دستمزد بعداً تنظیم می‌شود.",c.message.chat.id,c.message.message_id,theft_assistants_markup(owner))
...
def theft_buy(c):
    owner=check_panel_owner(c)
    if owner is None:return
    k=c.data.split("|")[1];price=THEFT_ITEM_PRICES.get(k)
    if price is None:bot.answer_callback_query(c.id,"قیمت این وسیله هنوز تنظیم نشده.",show_alert=True);return
    if get_balance(owner)<price:bot.answer_callback_query(c.id,"❌ موجودی کافی نیست.",show_alert=True);return
    if update_diamonds(owner,-price) is None or not _theft_add_item(owner,k):
        if price:update_diamonds(owner,price)
        bot.answer_callback_query(c.id,"❌ خرید انجام نشد؛ مبلغ برگشت داده شد.",show_alert=True);return
    bot.answer_callback_query(c.id,"✅ خرید انجام شد.")

@bot.callback_query_handler(func=lambda c:c.data.startswith("theftnoop|"))
def theft_noop(c):
    owner=check_panel_owner(c)
    if owner is not None:bot.answer_callback_query(c.id,"دستیار را از پنل دزدی انتخاب کن.")

@bot.message_handler(func=lambda m:text_is(m,"فروشگاه") and not _has_pending_step(m))
def theft_store_message(message):
    if not get_user(message.from_user.id):create_user(message.from_user.id,get_display_name(message.from_user))
    show_or_edit_panel(message.chat.id,message.from_user.id,"theft_store","🛒 فروشگاه دزدی\n\n✓ | دسته مد نظر را انتخاب کنید.",theft_store_markup(message.from_user.id))
