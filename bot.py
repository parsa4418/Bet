...
def colored_button(*args, **kwargs):
    ...
    danger_words = (
        "لغو", "انصراف", "خیر", "بستن", "حذف", "پاک", "رد", "بازگشت",
...
def start_bet_flow(message, amount):
    ...
    markup = types.InlineKeyboardMarkup()
    markup.row(
        colored_button("❌ لغو شرط", callback_data=f"cancel|{bet_id}"),
        colored_button("✅ پیوستن به شرط", callback_data=f"join|{bet_id}"),
    )
    markup.row(colored_button("🤖 شرط با ربات", callback_data=f"bot|{bet_id}"))
