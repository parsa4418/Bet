...
def show_or_edit_panel(chat_id, user_id, panel, text, reply_markup=None):
    """اگه این فراخوانی از یه پیامِ متنیِ کاربر (مثل «موجودی») اومده باشه، طبق
    خواسته‌ی کاربر همیشه یه پیام جدید (که ریپلای روی پیام خودشه) می‌فرستیم،
    نه اینکه پنل قبلی رو ادیت کنیم. اگه از دکمه (callback) اومده باشه، طبق
    روال قبلی همون پنل رو ادیت می‌کنیم تا چت شلوغ نشه."""
    ctx_chat_id = getattr(_reply_ctx, "chat_id", None)
    ctx_message_id = getattr(_reply_ctx, "message_id", None)
    force_new = bool(ctx_message_id) and str(ctx_chat_id) == str(chat_id)
