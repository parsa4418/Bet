...
def mine_panel_entry(message):
    ...
    bot.register_next_step_handler(msg, mine_bet_step, message.from_user.id)

def mine_bet_step(message, expected_user_id):
    if message.from_user.id != expected_user_id:
        # پیام از یه نفر دیگه بود؛ نادیده می‌گیریم ولی منتظر پیام خودِ کاربر می‌مونیم
        bot.register_next_step_handler(message, mine_bet_step, expected_user_id)
        return
    amount = parse_amount(message.text)
    if amount is None:
        msg = bot.reply_to(message, "❌ مبلغ نامعتبره. یه عدد درست بفرست (مثلاً 100000 یا 100k):")
        bot.register_next_step_handler(msg, mine_bet_step, expected_user_id)
        return
    if get_balance(expected_user_id) < amount:
    ...
    active_mine_games[sent.message_id] = {
