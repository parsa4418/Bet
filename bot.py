...
TROPHIES = {
    "msg_100":       ("📝 تازه‌وارد",        "اولین قدم رو برداشتی!"),
    "msg_500":       ("🗣️ پرحرف",           "عاشق حرف زدنه!"),
    "msg_1000":      ("🎤 سخنران",          "کسی نمی‌تونه جلوش رو بگیره!"),
    "msg_5000":      ("📢 بلندگو",          "واقعاً پرحرفی!"),
    "ring_1":        ("💎 الماس‌یاب",        "اولین الماس رو پیدا کردی!"),
    "ring_10":       ("💎💎 جوینده الماس",   "حرفه‌ای شدی!"),
    "ring_50":       ("💎💎💎 شکارچی الماس", "افسانه‌ای!"),
    "ring_100":      ("👑 سلطان الماس",     "پادشاه الماس‌ها!"),
    "bet_10":        ("🎲 قمارباز",         "وارد بازی شدی!"),
    "bet_50":        ("🎲🎲 قمارباز حرفه‌ای", "حرفه‌ای شدی!"),
    "bet_100":       ("🎲🎲🎲 سلطان شرط",    "کسی نمی‌تونه تو رو شکست بده!"),
    "casino_10":     ("🎰 گردونه‌باز",       "عاشق گردونه‌ای!"),
    "casino_50":     ("🎰🎰 کازینو حرفه‌ای", "حرفه‌ای شدی!"),
    "casino_100":    ("🎰🎰🎰 سلطان کازینو", "پادشاه کازینو!"),
    "wealth_500k":   ("⭐ ستاره",           "سرمایه‌دار شدی!"),
    "wealth_1m":     ("🌟 فوق‌ستاره",        "میلیونر شدی!"),
    "tournament_win": ("🏆 قهرمان تورنومنت", "بهترینی!"),
    # جام‌های نادر (فقط از طریق جعبه شانس)
    "rare_star":     ("🌟 جام ستاره‌ای",    "جایزه ویژه جعبه شانس!"),
    "rare_rainbow":  ("🌈 جام رنگین‌کمان",  "جایزه ویژه جعبه شانس!"),
    "rare_unicorn":  ("🦄 جام تک‌شاخ",      "جایزه ویژه جعبه شانس!"),
    "rare_fire":     ("🔥 جام آتشین",       "جایزه ویژه جعبه شانس!"),
}
# آستانه‌های پیام / الماس انگشتر / شرط / کازینو برای اعطای خودکار جام
MSG_TROPHY_THRESHOLDS = [(100, "msg_100"), (500, "msg_500"), (1_000, "msg_1000"), (5_000, "msg_5000")]
RING_TROPHY_THRESHOLDS = [(1, "ring_1"), (10, "ring_10"), (50, "ring_50"), (100, "ring_100")]
BET_WIN_TROPHY_THRESHOLDS = [(10, "bet_10"), (50, "bet_50"), (100, "bet_100")]
CASINO_WIN_TROPHY_THRESHOLDS = [(10, "casino_10"), (50, "casino_50"), (100, "casino_100")]
WEALTH_TROPHY_THRESHOLDS = [(500_000, "wealth_500k"), (1_000_000, "wealth_1m")]
RARE_TROPHY_KEYS = ["rare_star", "rare_rainbow", "rare_unicorn", "rare_fire"]
...
LOOTBOX_PRIZES = [
    ...
    ("🎁 ۱ ماه نقش ویژه",    2,  "role",    None),
    ("💎 50,000,000 الماس (جکپات!)", 1, "diamond", 50_000_000),
]
LOOTBOX_TAGS = ["⭐ ستاره", "👑 پادشاه", "🎯 تک‌تیرانداز", "💎 الماس‌یاب", "🎲 قمارباز"]
LOOTBOX_ROLES = ["💎 الماس‌یاب ارشد", "👑 سلطان گروه", "🎰 کازینو حرفه‌ای"]
...
def register_message_xp(user_id, chat_id, display_name):
    ...
    try:
        ...
        add_xp(user_id, XP_PER_MESSAGE, chat_id=chat_id, display_name=display_name)
...
TAG_DURATION_SECONDS = 7 * 86400  # تگ ویژه جعبه شانس ۱ هفته اعتبار داره
...
def _apply_cosmetic(user_id, cosmetic_type, value):
    try:
        ...
        if cosmetic_type == "tag":
            cosmetics["tag"] = value
            cosmetics["tag_expires"] = int(time.time()) + TAG_DURATION_SECONDS
        elif cosmetic_type == "role":
            cosmetics["role"] = value
            cosmetics["role_expires"] = int(time.time()) + 30 * 86400
...
def get_active_tag(user_id):
    """تگ فعال کاربر رو برمی‌گردونه، یا None اگه نداره/منقضی شده."""
    try:
        progress = get_progress(user_id)
        cosmetics = progress.get("cosmetics") or {}
        tag = cosmetics.get("tag")
        expires = cosmetics.get("tag_expires")
        if not tag or not expires:
            return None
...
def lootbox_open(call):
    ...
    if kind == "diamond":
        ...
        desc = f"👑 تگ ویژه: {tag} (اعتبار ۱ هفته)"
    elif kind == "trophy":
        ...
        if available:
            ...
            desc = f"🏅 جام نادر: {TROPHIES[key][0]}"
        else:
            update_diamonds(user_id, 500_000)
            desc = "🏅 همه جام‌های نادر رو داری! به‌جاش 💎 500,000 الماس گرفتی."
    elif kind == "role":
        role = random.choice(LOOTBOX_ROLES)
        _apply_cosmetic(user_id, "role", role)
        desc = f"🎁 نقش ویژه (۱ ماه): {role}"
...
def text_help(message):
    bot.reply_to(message, f"""📖 راهنمای استفاده از بات
...
💎 موجودی: موجودی
💸 انتقال: انتقال الماس <مقدار> (روی پیام مقصد ریپلای کن)
🎲 شرط‌بندی: شرط بندی <مقدار>
🎰 کازینو: کازینو
🏦 بانک الماس: بانک
🎡 گردونه الماس: گردونه
💎 معدن الماس: معدن الماس
⛏ حفاری: حفاری
🏆 رتبه‌بندی: رنک
🏅 جام‌های من: جام ها
📦 جعبه شانس: جعبه شانس
...
💎 الماس انگشتر خودکار است و بعد از هر ۲۰۰ پیام گروه ظاهر می‌شود.
لقب فعال همه کاربران در پیام‌ها با قالب «ملقب به: ...» نمایش داده می‌شود.

گردونه هر {SPIN_COOLDOWN_HOURS} ساعت یک‌بار است و بین {SPIN_MIN:,} تا {SPIN_MAX:,} 💎 می‌دهد.
...
def handle_show_help(call):
    ...
    text = (
        ...
        "💎 الماس انگشتر خودکار است و بعد از هر ۲۰۰ پیام گروه ظاهر می‌شود.\n\n"
        "لقب فعال همه کاربران در پیام‌ها با قالب «ملقب به: ...» نمایش داده می‌شود.\n"
        f"گردونه هر {SPIN_COOLDOWN_HOURS} ساعت یک‌بار است و بین {SPIN_MIN:,} تا {SPIN_MAX:,} 💎 می‌دهد.\n"
        f"پاداش هر زیرمجموعه: {REFERRAL_BONUS:,} 💎"
    )
    safe_edit_message(text, call.message.chat.id, call.message.message_id, reply_markup=None)
...
def text_lootbox(message):
    ...
    if kind=="diamond": update_diamonds(uid,value)
        ...
        tag=random.choice(LOOTBOX_TAGS); _apply_cosmetic(uid,"tag",tag); desc=f"👑 تگ ویژه: {tag} (اعتبار ۱ هفته)"
    elif kind=="trophy":
        available=[k for k in RARE_TROPHY_KEYS if k not in get_user_trophies(uid)]
        if available:
            key=random.choice(available); award_trophy(uid,key,chat_id=None,display_name=get_display_name(message.from_user)); desc=f"🏅 جام نادر: {TROPHIES[key][0]}"
        else: update_diamonds(uid,500_000); desc="🏅 همه جام‌های نادر رو داری! به‌جاش 💎 500,000 الماس گرفتی."
    elif kind=="role":
        role=random.choice(LOOTBOX_ROLES); _apply_cosmetic(uid,"role",role); desc=f"🎁 نقش ویژه (۱ ماه): {role}"
