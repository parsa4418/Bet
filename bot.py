...
# ================== محدودیت‌های شرط‌بندی / کازینو / انتقال ==================
BET_MIN = 100
BET_MAX_BASE = 10_000
# سقف شرط بر اساس سطح کاربر بالاتر می‌ره (اولین آستانه‌ای که سطح کاربر بهش
# می‌رسه یا ازش رد می‌شه، اعمال می‌شه؛ لیست باید از سطح بالا به پایین مرتب باشه)
BET_MAX_BY_LEVEL = [
    (10, 50_000),
    (7,  25_000),
    (5,  20_000),
...
JEWELRY_GEMS = {
    "agate":      {"name": "عقیق",       "emoji": "🔴", "tier": "🟢 معمولی",   "price": 1_000},
    "turquoise":  {"name": "فیروزه",     "emoji": "🩵", "tier": "🟢 معمولی",   "price": 1_000},
    "amethyst":   {"name": "آمیتیست",    "emoji": "🟣", "tier": "🔵 کمیاب",    "price": 2_000},
    "garnet":     {"name": "گارنت",      "emoji": "❤️", "tier": "🔵 کمیاب",    "price": 2_000},
...
DIAMOND_HUNT_COSTS = [100, 200, 300]
...
LEVELS = [
    ...
    (3,  "الماس‌یاب کارآموز",    250,        1_000),
    (4,  "الماس‌یاب نیمه‌حرفه‌ای", 500,        2_000),
    (5,  "الماس‌یاب حرفه‌ای",     800,        3_500),
    (6,  "الماس‌یاب خبره",       1_200,      5_000),
    (7,  "استاد الماس‌یابی",      1_800,      7_500),
    (8,  "افسانه الماس‌یابی",     2_500,      10_000),
    (9,  "سلطان الماس",          3_500,      20_000),
    (10, "پادشاه الماس",         5_000,      50_000),
]
...
TROPHIES = {
    ...
    "rare_diamond":  ("💎 جام الماس",        "جایزه ویژه جعبه شانس! همراهش ۵,۰۰۰ 💎 هدیه می‌گیری."),
}
...
WEALTH_TROPHY_THRESHOLDS = [(50_000, "wealth_1m"), (250_000, "wealth_10m"), (1_000_000, "wealth_1b")]
...
# جوایز الماسی عمداً کوچیک و پرنوسان‌ان (حتی منفی) که جعبه شانس واقعاً قمار باشه.
LOOTBOX_PRIZES = [
    ("diamond_neg", 15, "diamond", -100),
    ("diamond_zero", 15, "diamond", 0),
    ("diamond", 20, "diamond", 100),
    ("diamond", 15, "diamond", 250),
    ("diamond", 12, "diamond", 500),
    ("diamond", 10, "diamond", 1_000),
    ("diamond", 6,  "diamond", 2_000),
    ("👑 لقب ویژه", 4,  "tag",     None),
    ("🏅 جام نادر", 2,  "trophy",  None),
    ("diamond", 1,  "diamond", 5_000),
]
...
# ================== تنظیمات بانک ==================
BANK_OPENING_FEE = 2_000
BANK_INTEREST_RATE = 0.03
BANK_DAILY_INTEREST_MAX = 2_000
...
def update_diamonds(user_id, amount):
    try:
        data = _rpc("atomic_add_diamonds", {"p_user_id": int(user_id), "p_delta": int(amount)})
        return data is not None
    except Exception as e:
        logging.error(f"خطا در update_diamonds/RPC: {e}")
        return False
...
# ظرفیت انبار در هر سطح (حداکثر لول ۱۰ فقط ۲۰۰۰ الماس):
...
# سرعت دستگاه حفاری (ثانیه به‌ازای هر الماس؛ حداکثر سرعت یعنی حداقل ثانیه، که
# نباید از 0.5 ثانیه سریع‌تر بشه)
...
FACTORY_XP_PER_HARVEST_UNIT = 10
FACTORY_XP_NEEDED_PER_LEVEL = 10
...
def change_loan_balance(user_id, delta):
    try:
        _rpc("atomic_change_loan_balance", {"p_user_id": int(user_id), "p_delta": int(delta)})
        return True
    except Exception as e:
        logging.error(f"خطا در change_loan_balance/RPC: {e}")
        return False
...
def get_max_bet(user_id):
    """سقف مبلغ شرط‌بندی/کازینو رو بر اساس سطح کاربر برمی‌گردونه. هرچی سطح
    بالاتر بره، سقف شرط هم بالاتر می‌ره (طبق BET_MAX_BY_LEVEL)."""
    try:
        level = int(get_progress(user_id).get("level", 1) or 1)
    except Exception:
        level = 1
    for min_level, max_bet in BET_MAX_BY_LEVEL:
        if level >= min_level:
            return max_bet
    return BET_MAX_BASE
...
def _lootbox_diamond_desc(value):
    """متن نمایش جایزه‌ی الماسی جعبه شانس رو بر اساس مثبت/صفر/منفی بودنش می‌سازه."""
    if value > 0:
        return f"افزودن {value:,} الماس 💎"
    if value < 0:
        return f"کم کردن {abs(value):,} الماس 😂"
    return "0 الماس"
...
def lootbox_open(call):
    ...
    if kind == "diamond":
        if value < 0:
            deduct = min(abs(value), get_balance(user_id))
            update_diamonds(user_id, -deduct)
        elif value > 0:
            update_diamonds(user_id, value)
        desc = _lootbox_diamond_desc(value)
    elif kind == "tag":
        ...
        if available:
            ...
            update_diamonds(user_id, 1_000)
            desc = "🏅 جام الماس رو قبلاً داری! به‌جاش 💎 1,000 الماس گرفتی."
...
def remember_panel(chat_id, user_id, panel_key, message_id):
    try:
        _rpc("atomic_set_panel", {
            "p_chat_id": int(chat_id),
            "p_user_id": int(user_id),
            "p_panel_key": str(panel_key),
            "p_message_id": int(message_id),
        })
    except Exception as e:
        logging.warning(f"ثبت پنل {panel_key} انجام نشد: {e}")
...
def bank_confirmation_callback(call):
    ...
    if action == "deposit":
        if get_balance(user_id) < amount:
            _clear_bank_confirmation(user_id)
            bot.answer_callback_query(call.id, "موجودی الماس کافی نیست ❌", show_alert=True)
            safe_edit_message("❌ موجودی الماس شما دیگر برای این واریز کافی نیست.", call.message.chat.id, call.message.message_id, reply_markup=None)
            auto_return_to_bank(call.message.chat.id, call.message.message_id, user_id, 3)
            return
        apply_bank_interest(user_id)
        update_diamonds(user_id, -amount)
        if not change_bank_balance(user_id, amount):
            update_diamonds(user_id, amount)
            _clear_bank_confirmation(user_id)
        ...
        if get_bank_balance(user_id) < amount:
            _clear_bank_confirmation(user_id)
            bot.answer_callback_query(call.id, "موجودی بانک کافی نیست ❌", show_alert=True)
            safe_edit_message("❌ موجودی بانک شما دیگر برای این برداشت کافی نیست.", call.message.chat.id, call.message.message_id, reply_markup=None)
            auto_return_to_bank(call.message.chat.id, call.message.message_id, user_id, 3)
            return
        if not change_bank_balance(user_id, -amount):
            _clear_bank_confirmation(user_id)
...
def bank_deposit_step(message, expected_user_id):
    ...
    if amount is None:
        ...
        register_timed_next_step_handler(msg, bank_deposit_step, expected_user_id, expected_user_id=expected_user_id)
        return
    if amount <= 0:
    ...
    if get_balance(expected_user_id) < amount:
        ...
        return
    confirmation = bot.reply_to(message, "آیا از واریز اطمینان دارید؟\n\n" f"مبلغ درحال واریز: {amount:,} 💎", reply_markup=_bank_confirmation_markup(expected_user_id))
    if confirmation:
        _store_bank_confirmation(expected_user_id, "deposit", amount, message.chat.id, confirmation.message_id)
...
def bank_withdraw_step(message, expected_user_id):
    ...
    if amount is None:
        msg = bot.reply_to(message, "❌ فرمت درست نیست. مثال: 500000 یا 500k یا 500کا یا 12.5میل")
        register_timed_next_step_handler(msg, bank_withdraw_step, expected_user_id, expected_user_id=expected_user_id)
        return
    if amount <= 0:
        msg = bot.reply_to(message, "❌ مبلغ باید بیشتر از صفر باشد.")
        register_timed_next_step_handler(msg, bank_withdraw_step, expected_user_id, expected_user_id=expected_user_id)
    ...
    if get_bank_balance(expected_user_id) < amount:
        ...
        return
    confirmation = bot.reply_to(message, "آیا از برداشت اطمینان دارید؟\n\n" f"مبلغ درحال برداشت: {amount:,} 💎", reply_markup=_bank_confirmation_markup(expected_user_id))
    if confirmation:
        _store_bank_confirmation(expected_user_id, "withdraw", amount, message.chat.id, confirmation.message_id)
...
def loan_amount_step(message, expected_user_id, remaining):
    ...
    if amount is None:
        ...
        register_timed_next_step_handler(msg, loan_amount_step, expected_user_id, remaining, expected_user_id=expected_user_id)
        return
...
def text_help(message):
    bot.reply_to(
        message,
        "هر بخشی که نیاز به راهنمایی دارید رو انتخاب کنید‌.🌹",
        reply_markup=help_main_markup(message.from_user.id)
    )
...
def perform_transfer(sender_id, target_id, amount):
    ...
    if amount > TRANSFER_MAX:
        return False, f"حداکثر مبلغ انتقال {TRANSFER_MAX:,} 💎 هست."
...
def start_bet_flow(message, amount):
    ...
    if amount <= 0:
        ...
        return
    if amount < BET_MIN:
        bot.reply_to(message, f"حداقل مبلغ شرط‌بندی {BET_MIN:,} 💎 هست.")
        return
    max_bet = get_max_bet(user_id)
    if amount > max_bet:
        bot.reply_to(message, f"حداکثر مبلغ شرط‌بندی برای سطح فعلیت {max_bet:,} 💎 هست.")
        return
    if get_balance(user_id) < amount:
...
CASINO_BET_PRESETS = [100, 500, 1000, 5000, 10000]
...
def casino_bet_select(call):
    ...
    if not get_user(user.id):
        bot.answer_callback_query(call.id, "اول /start بزن.", show_alert=True)
        return
    max_bet = get_max_bet(user.id)
    if amount > max_bet:
        bot.answer_callback_query(call.id, f"حداکثر مبلغ شرط برای سطح فعلیت {max_bet:,} 💎 هست.", show_alert=True)
        return
...
def mine_panel_entry(message):
    ...
    register_timed_next_step_handler(msg, mine_bet_step, message.from_user.id, msg.message_id, expected_user_id=message.from_user.id)
...
def mine_bet_step(message, expected_user_id, prompt_msg_id):
    ...
    amount = parse_amount(message.text)
    if amount is None:
        msg = bot.reply_to(message, "❌ مبلغ نامعتبره. یه عدد درست بفرست (مثلاً 500 یا 1k):")
        register_timed_next_step_handler(msg, mine_bet_step, expected_user_id, prompt_msg_id, expected_user_id=expected_user_id)
        return
    if amount < BET_MIN:
        msg = bot.reply_to(message, f"حداقل مبلغ شرط {BET_MIN:,} 💎 هست. دوباره بفرست:")
        register_timed_next_step_handler(msg, mine_bet_step, expected_user_id, prompt_msg_id, expected_user_id=expected_user_id)
        return
    max_bet = get_max_bet(expected_user_id)
    if amount > max_bet:
        msg = bot.reply_to(message, f"حداکثر مبلغ شرط برای سطح فعلیت {max_bet:,} 💎 هست. دوباره بفرست:")
        register_timed_next_step_handler(msg, mine_bet_step, expected_user_id, prompt_msg_id, expected_user_id=expected_user_id)
        return
    if get_balance(expected_user_id) < amount:
...
def factory_production_step(message, expected_user_id, panel_msg_id):
    ...
    if amount is None or amount <= 0:
        ...
        register_timed_next_step_handler(msg, factory_production_step, expected_user_id, panel_msg_id, expected_user_id=expected_user_id)
        return
    if amount > stored:
...
def casino_custom_amount_step(message, game_key, expected_user_id, panel_msg_id):
    ...
    if amount is None:
        ...
        register_timed_next_step_handler(msg, casino_custom_amount_step, game_key, expected_user_id, panel_msg_id, expected_user_id=expected_user_id)
        return
...
def handle_dice_throw(message):
    ...
    with casino_lock:
        for msg_id, game in active_casino_games.items():
            ...
            if not reply or reply.message_id != msg_id or not getattr(reply.from_user, "is_bot", False):
                continue
            emoji = CASINO_GAMES[game["game"]]
...
def text_lootbox(message):
    ...
    if kind=="diamond":
        if value < 0:
            deduct = min(abs(value), get_balance(uid))
            update_diamonds(uid, -deduct)
        elif value > 0:
            update_diamonds(uid, value)
        desc = _lootbox_diamond_desc(value)
    elif kind=="tag":
        ...
        else: update_diamonds(uid,1_000); desc="🏅 جام الماس رو قبلاً داری! به‌جاش 💎 1,000 الماس گرفتی."
...
def _send_rank(message):
    ...
    bot.reply_to(message, text, reply_markup=_rank_markup(1, total, message.from_user.id))

@bot.message_handler(commands=["rank", "رتبه‌بندی"])
def cmd_rank(message):
    _send_rank(message)

@bot.message_handler(func=lambda m: m.text and m.text.strip() in ["رنک", "رتبه بندی", "رتبه‌بندی"])
...
def _start_jewelry_build(call, user_id, qty):
    try:
        ...
        # رزرو اتمیک واقعی نیاز به RPC دارد؛ اینجا بلافاصله موجودی را کم می‌کنیم
        # و سپس سفارش‌ها را ثبت می‌کنیم تا هر الماس دقیقاً یک‌بار مصرف شود.
        ...
        for i in range(qty):
            ...
            payload = {
                ...
                "quantity": 1,
                "gem_key": gem_key,
                "payout_total": gem["price"],
                "finish_at": finish_at.isoformat(),
                "claimed": False,
            }
            resp = supabase.table("jewelry_orders").insert(payload).execute()
            if resp.data:
                inserted.extend(resp.data)

        if len(inserted) != qty:
            # اگر ثبت ناقص شد، تا حد ممکن الماس‌های مصرف‌نشده را برگردان.
            missing = qty - len(inserted)
            if missing > 0:
                supabase.table("users").update({"ring_diamonds": new_balance + missing}).eq("user_id", user_id).execute()
            if getattr(call, "id", None):
                bot.answer_callback_query(call.id, "ثبت سفارش کامل نشد؛ دوباره تلاش کن.", show_alert=True)
            else:
                bot.send_message(call.message.chat.id, "❌ ثبت سفارش کامل نشد؛ دوباره تلاش کن.")
            return
...
def text_jewelry(message):
    ...
    active = _jewelry_active_orders(user_id)
    if active:
        bot.reply_to(message, _jewelry_active_text(active), reply_markup=None)
        return
    ready = _jewelry_ready_orders(user_id)
    if ready:
        text, total = _jewelry_ready_text(ready)
        bot.reply_to(message, text, reply_markup=_jewelry_ready_markup(user_id, total))
    else:
        bot.reply_to(message, _jewelry_build_text(), reply_markup=_jewelry_build_markup(user_id))
...
def jewelry_custom_amount(message):
    ...
    if qty is None or qty <= 0:
        ...
        bot.reply_to(message, "❌ تعداد نامعتبره. فقط عدد مثبت بفرست؛ مثلاً 10")
        return
    available = int((get_user(user_id) or {}).get("ring_diamonds", 0) or 0)
    if available < qty:
        JEWELRY_PENDING_CUSTOM[user_id] = panel
        bot.reply_to(message, f"❌ الماس انگشتر کافی نداری. موجودی: {available:,} 💎")
        return
...
def admin_panel(call):
    ...
    markup.add(
        types.InlineKeyboardButton("➕ افزودن الماس به همه", callback_data="admin_add_diamond_all"),
        types.InlineKeyboardButton("➖ کم کردن الماس از همه", callback_data="admin_remove_diamond_all"),
    )
    markup.add(types.InlineKeyboardButton("🏠 منوی اصلی", callback_data=f"mainmenu|{owner_id}"))
    safe_edit_message(text, call.message.chat.id, call.message.message_id, reply_markup=markup)
...
@bot.message_handler(func=lambda m: m.text and m.text.strip() == "مدیریت")
...
def admin_add_diamond_all_prompt(call):
    if call.from_user.id not in ADMIN_IDS:
        ...
        return
    bot.answer_callback_query(call.id)
    text = "➕ لطفاً مقدار الماسی که می‌خوای به همه‌ی کاربرا اضافه بشه رو وارد کن:\nمثال: `100` یا `1k`"
    safe_edit_message(text, call.message.chat.id, call.message.message_id, reply_markup=None)
...
def admin_add_diamond_all_execute(message):
    ...
    if amount is None or amount <= 0:
        msg = bot.reply_to(message, "❌ مبلغ نامعتبره. یه عدد درست بفرست (مثلاً 100 یا 1k):")
        register_timed_next_step_handler(msg, admin_add_diamond_all_execute, expected_user_id=message.from_user.id)
        return
    try:
        users = supabase.table("users").select("user_id").execute()
        if not users.data:
        ...
        for user in users.data:
            try:
                update_diamonds(user["user_id"], amount)
                count += 1
            except Exception:
                continue
        bot.reply_to(message, f"✅ {amount:,} 💎 به {count} کاربر اضافه شد.")
    except Exception as e:
        logging.error(f"خطا در افزودن الماس همگانی: {e}")
        bot.reply_to(message, f"❌ خطا: {e}")

# ---------- کم کردن الماس از همه (مدیریت) ----------
...
def admin_remove_diamond_all_prompt(call):
    if call.from_user.id not in ADMIN_IDS:
        ...
        return
    bot.answer_callback_query(call.id)
    text = "➖ لطفاً مقدار الماسی که می‌خوای از همه‌ی کاربرا کم بشه رو وارد کن:\nمثال: `100` یا `1k`"
    safe_edit_message(text, call.message.chat.id, call.message.message_id, reply_markup=None)
...
def admin_remove_diamond_all_execute(message):
    ...
    if amount is None or amount <= 0:
        msg = bot.reply_to(message, "❌ مبلغ نامعتبره. یه عدد درست بفرست (مثلاً 100 یا 1k):")
        register_timed_next_step_handler(msg, admin_remove_diamond_all_execute, expected_user_id=message.from_user.id)
        return
    try:
        users = supabase.table("users").select("user_id").execute()
        if not users.data:
        ...
        for user in users.data:
            try:
                uid = user["user_id"]
                deduct = min(amount, get_balance(uid))
                if deduct > 0:
                    update_diamonds(uid, -deduct)
                count += 1
            except Exception:
                continue
        bot.reply_to(message, f"✅ تا سقف {amount:,} 💎 از {count} کاربر کم شد (هرکس کمتر از این داشت، فقط همون مقدار موجودش کم شد).")
    except Exception as e:
        logging.error(f"خطا در کم‌کردن الماس همگانی: {e}")
        bot.reply_to(message, f"❌ خطا: {e}")
