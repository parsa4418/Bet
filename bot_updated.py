...
def casino_bet_select(call):
    ...
    text = (
        ...
        f"⏳ منتظر بازیکن دوم... (فرصت پیوستن: {JOIN_TIMEOUT_SECONDS} ثانیه)"
    )
...
def text_lootbox(message):
    ...
    if get_balance(uid)<LOOTBOX_COST:
        show_or_edit_panel(message.chat.id, uid, "lootbox", f"📦 برای باز کردن جعبه شانس {LOOTBOX_COST:,} 💎 لازم داری.", None); return
    desc,kind,value=_pick_lootbox_prize()
    if kind=="diamond" and ATOMIC_DB_MODE:
        try:
            ...
            logging.error(f"خطا در جعبه شانس اتمیک: {e}")
            show_or_edit_panel(message.chat.id, uid, "lootbox", "❌ باز کردن جعبه انجام نشد.", None)
            return
    else:
