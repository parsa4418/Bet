...
_telegram_api_lock = threading.RLock()
...
def _wait_for_telegram_slot(chat_id=None):
    ...
    _telegram_next_allowed_at = sent_at + _TELEGRAM_GLOBAL_MIN_INTERVAL
    if chat_id is not None:
        _telegram_last_request_by_chat[str(chat_id)] = sent_at + _TELEGRAM_CHAT_MIN_INTERVAL
    ...
    while True:
        with _telegram_api_lock:
            ...
            try:
                ...
                # Lock is released before sleeping so webhook processing can
                # continue; the next Telegram operation will respect the gate.
