...
# ===== RTL ENABLED WRAPPERS (AUTO-INJECTED) =====
RTL = "\u200F"

# preserve original methods (if previously saved, reuse)
try:
    _original_send_message
except NameError:
    _original_send_message = getattr(bot, "send_message")

_original_edit_message_text = getattr(bot, "edit_message_text", None)
_original_send_photo = getattr(bot, "send_photo", None)
_original_edit_message_caption = getattr(bot, "edit_message_caption", None)

def _fix_text(text):
    if isinstance(text, str) and not text.startswith(RTL):
        return RTL + text
    return text

def _send_message_auto_reply(chat_id, text, *args, **kwargs):
    # ensure RTL prefix for textual content
...
# wrap edit_message_text if available
if _original_edit_message_text:
...
# wrap send_photo to fix caption
if _original_send_photo:
    def _send_photo_rtl(chat_id, photo, *args, **kwargs):
        if "caption" in kwargs and isinstance(kwargs["caption"], str):
            kwargs["caption"] = _fix_text(kwargs["caption"])
        return _original_send_photo(chat_id, photo, *args, **kwargs)
    bot.send_photo = _send_photo_rtl

# wrap edit_message_caption if available
if _original_edit_message_caption:
    def _edit_caption_rtl(*args, **kwargs):
        if "caption" in kwargs and isinstance(kwargs["caption"], str):
            kwargs["caption"] = _fix_text(kwargs["caption"])
        return _original_edit_message_caption(*args, **kwargs)
    bot.edit_message_caption = _edit_caption_rtl
...
def bank_markup(user_id):
    ...
    markup.row(colored_button("🏠 بازگشت به منو", callback_data=f"mainmenu|{user_id}"))
    return markup
...
def bank_open_markup(user_id):
    ...
    markup.add(colored_button("🏠 منوی اصلی", callback_data=f"mainmenu|{user_id}"))
    return markup
...
def help_main_markup(user_id):
    ...
    markup.row(
        ...
        colored_button("📦 جعبه شانس", callback_data=f"help_topic|{user_id}|lootbox")
    )
...
def start_bet_flow(message, amount):
    ...
    markup.row(colored_button("بازی با ربات ⏣", callback_data=f"bot|{bet_id}"))
...
def casino_games_keyboard(owner_id):
    ...
    for key, emoji in CASINO_GAMES.items():
        markup.add(colored_button(f"{emoji} {CASINO_GAME_NAMES[key]}", callback_data=f"cgame|{key}|{owner_id}"))
    markup.add(colored_button("🏠 منوی اصلی", callback_data=f"mainmenu|{owner_id}"))
    return markup
...
def casino_bet_select(call):
    ...
    markup = types.InlineKeyboardMarkup()
    markup.row(
        colored_button("❌ لغو بازی", callback_data=f"ccancel|{call.message.message_id}"),
        colored_button("✅ پیوستن به بازی", callback_data="cjoin"),
    )
    markup.row(colored_button("🤖 بازی با ربات", callback_data="cbotplay"))
...
def factory_main_markup(owner_id):
    ...
    markup.add(colored_button("🏠 بازگشت به منو", callback_data=f"mainmenu|{owner_id}"))
    return markup
...
def casino_custom_amount_step(message, game_key, expected_user_id, panel_msg_id):
    ...
    markup = types.InlineKeyboardMarkup()
    markup.row(
        colored_button("❌ لغو بازی", callback_data=f"ccancel|{panel_msg_id}"),
        colored_button("✅ پیوستن به بازی", callback_data="cjoin"),
    )
    markup.row(colored_button("🤖 بازی با ربات", callback_data="cbotplay"))
...
def show_balance(message):
    ...
    if message.reply_to_message:
        ...
        markup.add(colored_button("🏠 منوی اصلی", callback_data="mainmenu"))
...
def cmd_admin(message):
    ...
    markup.add(colored_button("➕ افزودن الماس به همه", callback_data="admin_add_diamond_all"))
    markup.add(colored_button("➖ کم کردن الماس از همه", callback_data="admin_remove_diamond_all"))
    markup.add(colored_button("🏠 منوی اصلی", callback_data=f"mainmenu|{message.from_user.id}"))
...
# ===== BLOCK "PLAY WITH BOT" MODE =====
@bot.callback_query_handler(func=lambda call: bool(call.data) and ("|bot" in call.data or "bot" in call.data))
def _block_bot_mode_callback(call):
    try:
        bot.answer_callback_query(call.id, "⛔ بازی با ربات غیرفعال شده", show_alert=True)
    except Exception:
        pass

@bot.message_handler(func=lambda m: getattr(m, "text", "") and "بازی" in m.text and "ربات" in m.text)
def _block_bot_mode_text(m):
    try:
        bot.reply_to(m, RTL + "⛔ بازی با ربات غیرفعال شده")
    except Exception:
        pass

# helper to clean inline keyboards (remove buttons that reference bot mode)
def _clean_inline_markup(markup):
    try:
        # markup may be types.InlineKeyboardMarkup or similar with .inline_keyboard
        ...
        for row in kb:
            ...
            for btn in row:
                ...
                if "|bot" in str(cb) or "بازی با ربات" in str(txt) or ("ربات" in str(txt) and "بازی" in str(txt)):
                    continue
                # recreate button preserving callback_data or url
                if getattr(btn, "url", None):
                    new_row.append(types.InlineKeyboardButton(text=btn.text, url=btn.url))
                else:
                    new_row.append(types.InlineKeyboardButton(text=btn.text, callback_data=btn.callback_data))
            if new_row:
...
# try to clean markup when any callback is received
@bot.callback_query_handler(func=lambda call: True)
def _cleanup_on_any_callback(call):
    try:
        markup = getattr(call.message, "reply_markup", None)
        new = _clean_inline_markup(markup)
        if new:
            try:
                bot.edit_message_reply_markup(call.message.chat.id, call.message.message_id, reply_markup=new)
            except Exception:
                pass
    except Exception:
        pass
...
# ===== REMOVE "BACK TO MENU" BUTTONS (EXCEPT HELP & ACCOUNT) =====
def _clean_back_buttons(markup, context_text=None):
    try:
        kb = getattr(markup, "inline_keyboard", None)
        if not kb:
            return None
        ...
        for row in kb:
            ...
            for btn in row:
                ...
                # keep in help/account contexts
                if context_text and (("راهنما" in context_text) or ("حساب" in context_text)):
                    keep = True
                else:
                    # drop back-to-menu buttons
                    if ("بازگشت به منو" in txt) or ("menu" in cb and "back" in cb):
                        keep = False
                    else:
                        keep = True
                if keep:
...
# apply cleaning on every callback to update keyboards
@bot.callback_query_handler(func=lambda call: True)
def _cleanup_back_buttons_on_callback(call):
    try:
        msg = getattr(call, "message", None)
        if not msg:
            return
        markup = getattr(msg, "reply_markup", None)
        ctx_text = getattr(msg, "text", "") or getattr(msg, "caption", "")
        new = _clean_back_buttons(markup, context_text=ctx_text)
        if new:
            try:
                bot.edit_message_reply_markup(msg.chat.id, msg.message_id, reply_markup=new)
            except Exception:
                pass
    except Exception:
        pass
# ===== END REMOVE BACK BUTTONS =====
