...
# ================== کنترل مرکزی ویرایش پیام‌های Telegram ==================
# تمام editها از یک مسیر عبور می‌کنند تا چند Thread/Timer هم‌زمان به Telegram
# درخواست نزنند. این کار جلوی «هجوم مجدد» بعد از 429 را هم می‌گیرد.
_edit_api_lock = threading.RLock()
_edit_last_request_at = 0.0
_EDIT_MIN_INTERVAL = 0.06  # سقف محافظه‌کارانه برای فاصله‌ی درخواست‌های edit
_edit_success_cache = {}
...
def _remember_edit_success(key, fingerprint):
    ...
    if len(_edit_success_cache) > _EDIT_CACHE_MAX:
        # پاک‌سازی ساده و کم‌هزینه؛ فقط بخشی از قدیمی‌ترین ورودی‌ها حذف می‌شود.
...
def _is_rate_limit_error(exc):
    text = str(exc).lower()
    return _extract_retry_after(exc) is not None or "too many requests" in text or "429" in text

def _edit_wait_turn():
    global _edit_last_request_at
    now = time.monotonic()
    wait = _EDIT_MIN_INTERVAL - (now - _edit_last_request_at)
    if wait > 0:
        time.sleep(wait)
    _edit_last_request_at = time.monotonic()

def _start_daemon_timer(delay, callback, *args):
    """Timer استاندارد بات: daemon است تا Timerها مانع shutdown/redeploy نشوند."""
...
def safe_edit_message(text, chat_id, message_id, reply_markup=None, parse_mode=None, retries=2):
    """ویرایش مرکزی و قابل‌اعتماد پیام.

    ویژگی‌ها:
    - اگر متن/کیبورد دقیقاً همان چیزی باشد که همین بات با موفقیت ثبت کرده، هیچ
      درخواست اضافه‌ای به Telegram ارسال نمی‌شود.
    - تمام editها سراسری serialize می‌شوند تا Thread/Timerها روی هم نیفتند.
    - در 429 دقیقاً به اندازه‌ی retry_after (به‌علاوه‌ی یک ثانیه‌ی حاشیه) صبر می‌شود؛
      نه 0.35 ثانیه و نه retry storm.
    - خطاهای موقتی شبکه چند بار retry می‌شوند.
    - خطاهای دائمی مثل نبودن پیام یا غیرقابل‌ویرایش بودن، بی‌جهت retry نمی‌شوند.
    """
    global _edit_last_request_at
        ...
        while True:
            try:
                ...
                if "message to edit not found" in err_text or "message can't be edited" in err_text:
                    logging.warning(f"پیام {message_id} قابل ویرایش نیست: {e}")
                    return False

                if _is_rate_limit_error(e):
                    rate_limit_attempts += 1
                    retry_after = _extract_retry_after(e) or 5
                    if rate_limit_attempts > 8:
                        logging.error(f"429 مکرر برای ویرایش پیام {message_id} بعد از {rate_limit_attempts - 1} تلاش: {last_err}")
                        return False
                    wait_seconds = retry_after + 1
                    logging.warning(
                        f"Telegram rate limit هنگام ویرایش پیام {message_id}; "
                        f"{wait_seconds} ثانیه صبر می‌کنیم (تلاش {rate_limit_attempts}/8)."
                    )
...
def safe_edit_message_reply_markup(chat_id, message_id, reply_markup=None, retries=2):
    """ویرایش امن فقط کیبورد، با همان صف/قفل/429 handling مرکزی."""
    ...
    with _edit_api_lock:
        ...
        while True:
            try:
                ...
                if "message to edit not found" in err_text or "message can't be edited" in err_text:
                    logging.warning(f"کیبورد پیام {message_id} قابل ویرایش نیست: {e}")
                    return False
                ...
                if _is_rate_limit_error(e):
                    ...
                    retry_after = _extract_retry_after(e) or 5
                    if rate_limit_attempts > 8:
                        logging.error(f"429 مکرر برای کیبورد پیام {message_id}: {last_err}")
                        return False
                    wait_seconds = retry_after + 1
                    logging.warning(
                        f"Telegram rate limit هنگام ویرایش کیبورد پیام {message_id}; "
                        f"{wait_seconds} ثانیه صبر می‌کنیم (تلاش {rate_limit_attempts}/8)."
                    )
                    time.sleep(wait_seconds)
                    continue
                ...
                logging.error(f"خطا در ویرایش کیبورد پیام {message_id}: {last_err}")
                return False
