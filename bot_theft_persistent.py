...
# ================== سیستم دزدی ==================
# موجودی وسایل داخل user_progress.cosmetics ذخیره می‌شود؛ جدول جدیدی لازم نیست.
THEFT_ITEM_PRICES = {
    "bag": 5000,
    "mask": 1000,
    "gloves": 1000,
...
# وسایل هر نوع عملیات دقیقاً مطابق پنل موردنظر
THEFT_ITEMS = {
    "bank": [
        ("bag", "👜 کیف"),
        ("mask", "ماسک"),
        ...
        ("flashlight", "🔦 چراغ قوه"),
        ("car", "🚗 ماشین"),
        ("metal_detector", "🔎 فلز یاب"),
        ("bomb", "💣 بمب"),
        ("laser_cutter", "🔴 برش‌دهنده لیزری"),
        ("hacker_pc", "💻 کامپیوتر هکر"),
    ],
    "house": [
        ("bag", "👜 کیف"),
        ("flashlight", "🔦 چراغ قوه"),
        ("car", "🚗 ماشین"),
        ("lockpick", "🔓 دستگاه قفل باز کن"),
        ("metal_detector", "🔎 فلز یاب"),
        ("hacker_pc", "💻 کامپیوتر هکر"),
    ],
}
...
# دستیارها کاملاً فرضی‌اند و از اعضای گروه انتخاب نمی‌شوند.
THEFT_ASSISTANTS = [
    ("camera_hacker", "📹 هکر دوربین"),
    ("driver", "🚗 راننده ماشین"),
    ("safecracker", "🔐 صندوق باز کن"),
    ("locksmith", "🔑 کلید ساز"),
    ("bagman", "👜 حمال کیف"),
]
THEFT_ASSISTANT_SHARE_PERCENT = 10

# ریسک پلیس: هر وسیله/دستیار انتخاب‌شده 6٪ از ریسک کم می‌کند.
THEFT_POLICE_BASE_CHANCE = 90
THEFT_POLICE_REDUCTION_PER_ROLE = 6
THEFT_POLICE_MIN_CHANCE = 10
THEFT_HOUSE_MAX_PERCENT = 40
THEFT_BANK_MAX_PERCENT = 30
THEFT_MIN_MINUTES = 30
THEFT_MAX_MINUTES = 150
THEFT_NIGHT_WAIT_MINUTES = 60

# Supabase migration required for persistent theft jobs/cooldowns:
# CREATE TABLE IF NOT EXISTS theft_jobs (
#   id BIGSERIAL PRIMARY KEY,
#   chat_id BIGINT NOT NULL,
#   thief_id BIGINT NOT NULL,
#   target_id BIGINT NOT NULL,
#   theft_type TEXT NOT NULL CHECK (theft_type IN ('house','bank')),
#   stolen_amount BIGINT NOT NULL DEFAULT 0,
#   assistants_count INTEGER NOT NULL DEFAULT 0,
#   duration_seconds INTEGER NOT NULL DEFAULT 1800,
#   started_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
#   ends_at TIMESTAMPTZ NOT NULL,
#   cooldown_until TIMESTAMPTZ,
#   status TEXT NOT NULL DEFAULT 'active' CHECK (status IN ('active','cooldown','done')),
#   notified_thief BOOLEAN NOT NULL DEFAULT FALSE,
#   notified_target BOOLEAN NOT NULL DEFAULT FALSE,
#   created_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
# );
# CREATE INDEX IF NOT EXISTS idx_theft_jobs_status_ends ON theft_jobs(status, ends_at);
# CREATE INDEX IF NOT EXISTS idx_theft_jobs_user ON theft_jobs(chat_id, thief_id, status);


_theft_sessions = {}
_theft_lock = threading.RLock()
_theft_jobs = {}

def _theft_db_table():
    return supabase.table("theft_jobs")

def _persist_theft_job(chat_id, thief_id, target_id, theft_type, stolen_amount,
                       assistants_count, duration_seconds, ends_at):
    try:
        row = {
            "chat_id": int(chat_id),
            "thief_id": int(thief_id),
            "target_id": int(target_id),
            "theft_type": str(theft_type),
            "stolen_amount": int(stolen_amount),
            "assistants_count": int(assistants_count),
            "duration_seconds": int(duration_seconds),
            "ends_at": ends_at,
            "status": "active",
            "notified_thief": False,
            "notified_target": False,
        }
        res = _theft_db_table().insert(row).execute()
        data = getattr(res, "data", None) or []
        return data[0].get("id") if data else None
    except Exception:
        logger.exception("Failed to persist theft job")
        return None

def _set_theft_job_cooldown(job_id, cooldown_until):
    if not job_id:
        return False
    try:
        _theft_db_table().update({
            "status": "cooldown",
            "cooldown_until": cooldown_until,
        }).eq("id", job_id).execute()
        return True
    except Exception:
        logger.exception("Failed to persist theft cooldown")
        return False

def _finish_persisted_theft_job(job_id):
    if not job_id:
        return False
    try:
        _theft_db_table().update({"status": "done"}).eq("id", job_id).execute()
        return True
    except Exception:
        logger.exception("Failed to close theft job")
        return False

def _load_persisted_theft_jobs():
    try:
        res = (
            _theft_db_table()
            .select("*")
            .in_("status", ["active", "cooldown"])
            .execute()
        )
        return getattr(res, "data", None) or []
    except Exception:
        logger.exception("Failed to load persisted theft jobs")
        return []

_theft_cooldowns = {}


def _theft_inv(user_id):
    progress = get_progress(user_id)
    cosmetics = dict(progress.get("cosmetics") or {})
    inv = dict(cosmetics.get("theft_inventory") or {})
    inv.setdefault("items", {})
    return cosmetics, inv


def get_theft_inventory(user_id):
    try:
        return _theft_inv(user_id)[1]
    except Exception:
        return {"items": {}}


def _save_theft_inv(user_id, inv, cosmetics=None):
    try:
        if cosmetics is None:
            cosmetics = _theft_inv(user_id)[0]
        cosmetics["theft_inventory"] = inv
        supabase.table("user_progress").update({"cosmetics": cosmetics}).eq("user_id", user_id).execute()
        return True
    except Exception as e:
        logging.error(f"theft inventory save: {e}")
        return False


def _theft_add_item(user_id, key):
    cosmetics, inv = _theft_inv(user_id)
    items = dict(inv.get("items") or {})
    items[key] = int(items.get(key, 0) or 0) + 1
    inv["items"] = items
    return _save_theft_inv(user_id, inv, cosmetics)


def _theft_count(user_id, key):
    return int((get_theft_inventory(user_id).get("items") or {}).get(key, 0) or 0)


def _theft_consume_items(user_id, keys):
    cosmetics, inv = _theft_inv(user_id)
    items = dict(inv.get("items") or {})
    if any(int(items.get(k, 0) or 0) <= 0 for k in keys):
        return False
    for k in keys:
        items[k] = int(items.get(k, 0) or 0) - 1
    inv["items"] = items
    return _save_theft_inv(user_id, inv, cosmetics)


def _theft_item_name(key):
    for k, n in THEFT_SHOP_ITEMS:
        if k == key:
            return n
    return key


def _theft_assistant_name(key):
    return dict(THEFT_ASSISTANTS).get(key, key)


def _theft_member_ok(chat_id, user_id):
    try:
        status = bot.get_chat_member(chat_id, int(user_id)).status
        return status in ("creator", "administrator", "member", "restricted")
    except Exception as e:
        logging.warning(f"theft membership check: {e}")
        return False


def _theft_random_group_target(chat_id, thief_id):
    """از بین کاربران ثبت‌شده، یک عضو واقعی همین گروه را به‌صورت تصادفی انتخاب می‌کند.
    چون Telegram Bot API فهرست کامل اعضای گروه را به بات نمی‌دهد، اعضای قابل‌شناسایی
    از دیتابیس گرفته و عضویت تک‌تک آنها در همین گروه تأیید می‌شود.
    """
    try:
        rows = supabase.table("users").select("user_id").execute().data or []
    except Exception as e:
        logging.error(f"theft target query: {e}")
        return None
    candidates = []
    ids = []
    for row in rows:
        try:
            ...
            continue
        if uid == int(thief_id) or uid in ids:
            continue
        ids.append(uid)
    random.shuffle(ids)
    for uid in ids:
        if _theft_member_ok(chat_id, uid):
            candidates.append(uid)
            # چند گزینه کافی است و یکی به‌صورت تصادفی از بین آنها انتخاب می‌شود.
            if len(candidates) >= 10:
                break
    return random.choice(candidates) if candidates else None


def theft_store_markup(user_id):
    m = types.InlineKeyboardMarkup()
    m.add(colored_button("🧰 وسایل ها", callback_data=f"theftshop|items|{user_id}"))
    return m


def theft_store_items_markup(user_id):
    m = types.InlineKeyboardMarkup()
    inv = get_theft_inventory(user_id).get("items") or {}
    for key, name in THEFT_SHOP_ITEMS:
        price = THEFT_ITEM_PRICES.get(key)
        if price is None or price <= 0:
            price_text = "بزودی"
        else:
            price_text = f"{price:,} الماس"
        m.add(colored_button(
            f"🛒 {name} | {price_text} | موجودی {int(inv.get(key, 0) or 0)}",
            callback_data=f"theftbuy|{key}|{user_id}"
        ))
    m.add(colored_button("🔙 فروشگاه", callback_data=f"theftstore|{user_id}"))
    return m


def theft_main_markup(user_id):
    m = types.InlineKeyboardMarkup()
    m.row(
        colored_button("🏠 دزدی از خانه", callback_data=f"theftmode|house|{user_id}"),
        colored_button("🏦 دزدی از بانک", callback_data=f"theftmode|bank|{user_id}"),
    )
    return m


def theft_session_text(chat_id, user_id):
    with _theft_lock:
        s = dict(_theft_sessions.get((chat_id, user_id), {}))
    mode = s.get("mode", "house")
    items = s.get("items", [])
    aids = s.get("assistants", [])
    risk = max(
        THEFT_POLICE_MIN_CHANCE,
        THEFT_POLICE_BASE_CHANCE - THEFT_POLICE_REDUCTION_PER_ROLE * (len(items) + len(aids))
    )
    mx = THEFT_BANK_MAX_PERCENT if mode == "bank" else THEFT_HOUSE_MAX_PERCENT
    return (
        "◈ ━━━━━ 𝗧𝗵𝗲𝗳𝘁 ━━━━━ ◈\n"
        f"☻ | نوع دزدی : {'🏠 خانه' if mode == 'house' else '🏦 بانک'}\n\n"
        "🎯 | هدف : انتخاب تصادفی توسط بات\n"
        f"🧰 | وسایل انتخابی : {', '.join(_theft_item_name(k) for k in items) or 'انتخاب نشده'}\n"
        f"🧑‍🤝‍🧑 | دستیارها : {', '.join(_theft_assistant_name(k) for k in aids) or 'انتخاب نشده'}\n\n"
        f"💰 | حداکثر قابل دزدی : {mx}% موجودی هدف\n"
        f"🚨 | احتمال دستگیری : {risk}%\n\n"
        "✓ | هدف توسط بات و فقط از بین اعضای همین گروه انتخاب می‌شود.\n"
        "✓ | هر وسیله فقط برای یک عملیات مصرف می‌شود.\n"
        "✓ | سهم هر دستیار 10٪ است و سهم دستیارها به ادمین اول واریز می‌شود."
    )


def _theft_status(chat_id, user_id):
    key = (int(chat_id), int(user_id))
    now = time.time()
    with _theft_lock:
        job = _theft_jobs.get(key)
        if job:
            return "active", job
        until = _theft_cooldowns.get(key, 0)
        if until > now:
            return "cooldown", until
        if until:
            _theft_cooldowns.pop(key, None)
    return "ready", None


def _theft_wait_message(chat_id, user_id):
    status, data = _theft_status(chat_id, user_id)
    if status == "active":
        place = "بانک" if data.get("mode") == "bank" else "خونه"
        return (
            f"⏳ | صبر کن؛ الان داری از {place} مردم دزدی می‌کنی، صبر کن تا برگردی🤦‍♂️\n\n"
            "اصلا شاید دستگیر شی❌"
        )
    if status == "cooldown":
        remaining = max(0, int(data - time.time()))
        minutes = max(1, (remaining + 59) // 60)
        return f"❌ | صبر کن تا شب بشه❌\n\nتا شب : {minutes} دقیقه ⏳"
    return None


def theft_session_markup(user_id, chat_id):
    status, _ = _theft_status(chat_id, user_id)
    if status != "ready":
        return None
    with _theft_lock:
        s = _theft_sessions.get((chat_id, user_id), {})
    m = types.InlineKeyboardMarkup()
    if s.get("mode"):
        m.add(colored_button("🧰 انتخاب وسایل", callback_data=f"theftgear|{user_id}"))
        if s.get("items"):
            m.add(colored_button("🧑‍🤝‍🧑 انتخاب دستیارها", callback_data=f"theftassistants|{user_id}"))
        if s.get("items") and "assistants_done" in s:
            m.add(colored_button("🚨 شروع دزدی", callback_data=f"theftstart|{user_id}"))
    return m


def _theft_calculate_duration(stolen, max_stolen):
    if max_stolen <= 0:
        return THEFT_MIN_MINUTES
    ratio = max(0.0, min(1.0, stolen / max_stolen))
    return int(round(THEFT_MIN_MINUTES + (THEFT_MAX_MINUTES - THEFT_MIN_MINUTES) * ratio))


def _theft_finish(job_key):
    with _theft_lock:
        job = _theft_jobs.pop(job_key, None)
    if not job:
        return

    thief = int(job["thief_id"])
    target = int(job["target_id"])
    stolen = int(job["stolen"])
    items = list(job["items"])
    assistants = list(job["assistants"])
    risk = int(job["risk"])
    mode = job["mode"]
    target_name = job.get("target_name", "کاربر")

    # نتیجه پلیس در پایان زمان عملیات مشخص می‌شود.
    caught = random.random() < risk / 100
    first, second = sorted([thief, target])
    try:
        with _get_user_financial_lock(first):
            with _get_user_financial_lock(second):
                if caught:
                    # مبلغ رزرو شده به هدف برمی‌گردد.
                    if mode == "bank":
                        change_bank_balance(target, stolen)
                    else:
                        update_diamonds(target, stolen)
                    result = (
                        "🚔 دزدی لو رفت!\n\n"
                        "❌ | هیچ الماسی به دستت نرسید.\n"
                        "🔒 | وسایل این عملیات مصرف شدند.\n"
                        "⛓ | سیستم زندان در آپدیت بعدی اضافه می‌شود."
                    )
                else:
                    parts = 1 + len(assistants)
                    assistant_total = stolen * len(assistants) * THEFT_ASSISTANT_SHARE_PERCENT // 100
                    initiator = stolen - assistant_total
                    # برای اینکه سهم سازنده حداقل 50٪ بماند، 5 دستیار = 50٪.
                    if update_diamonds(thief, initiator) is None:
                        # تلاش برای برگشت رزرو
                        if mode == "bank":
                            change_bank_balance(target, stolen)
                        else:
                            update_diamonds(target, stolen)
                        raise RuntimeError("thief credit failed")
                    if assistant_total:
                        if not ADMIN_IDS or update_diamonds(ADMIN_IDS[0], assistant_total) is None:
                            update_diamonds(thief, -initiator)
                            if mode == "bank":
                                change_bank_balance(target, stolen)
                            else:
                                update_diamonds(target, stolen)
                            raise RuntimeError("admin assistant share credit failed")
                    result = (
                        "💰 دزدی با موفقیت انجام شد!\n\n"
                        f"$ | مبلغ دریافتی : {initiator:,}\n"
                        f"✓ | سهم دریافتی دستیار ها : {assistant_total:,}"
                    )
        with _theft_lock:
            _theft_cooldowns[(job_key[0], thief)] = time.time() + THEFT_NIGHT_WAIT_MINUTES * 60
            _theft_sessions.pop((job_key[0], thief), None)

        safe_send_message(thief, result)

        # اطلاع‌رسانی به قربانی فقط در صورت موفقیت دزدی.
        if not caught:
            thief_user = get_user(thief) or {}
            thief_first_name = (thief_user.get("first_name") or thief_user.get("username") or "کاربر").strip()
            if mode == "bank":
                victim_notice = (
                    "🚨 از بانک شما دزدی شد!\n\n"
                    f"مقدار : {stolen:,} الماس💎\n\n"
                    f"توسط : {thief_first_name}🗿"
                )
            else:
                victim_notice = (
                    "🚨 از شما دزدی شد!\n\n"
            ...
            # اگر قربانی قبلاً PV بات را باز کرده باشد، پیام مستقیم برایش می‌رود.
            safe_send_message(target, victim_notice)

        safe_send_message(thief, "❌ | صبر کن تا شب بشه❌\n\nتا شب : ۶۰ دقیقه ⏳")
        logging.info(
            f"theft finished thief={thief} target={target} amount={stolen} "
            f"mode={mode} caught={caught} duration={job.get('duration_minutes')}m"
        )
    except Exception as e:
        logging.error(f"theft finish failed: {e}")
        # در خطای غیرقابل‌جبران، رزرو را به هدف برگردان تا الماس گم نشود.
        try:
            if mode == "bank":
                change_bank_balance(target, stolen)
            else:
            ...
            logging.error(f"theft refund failed: {refund_error}")
        safe_send_message(thief, "❌ عملیات دزدی با خطای فنی مواجه شد و مبلغ رزرو‌شده برگشت داده شد.")


def _theft_run(chat_id, user_id):
    key = (chat_id, user_id)
    status, data = _theft_status(chat_id, user_id)
    if status == "active":
        place = "بانک" if data.get("mode") == "bank" else "خونه"
        return False, f"⏳ | صبر کن؛ الان داری از {place} مردم دزدی می‌کنی، صبر کن تا برگردی🤦‍♂️\\n\\nاصلا شاید دستگیر شی❌"
    if status == "cooldown":
        remaining = max(0, int(data - time.time()))
        minutes = max(1, (remaining + 59) // 60)
        return False, f"❌ | صبر کن تا شب بشه❌\\n\\nتا شب : {minutes} دقیقه ⏳"

    with _theft_lock:
        s = dict(_theft_sessions.get(key, {}))
        if s.get("starting"):
            return False, "⏳ | دزدی در حال شروع است؛ کمی صبر کن."
        if s:
            s["starting"] = True
            _theft_sessions[key] = s
    mode = s.get("mode", "house")
    items = list(s.get("items", []))
    assistants = list(s.get("assistants", []))

    if not items:
        with _theft_lock:
            if key in _theft_sessions:
                _theft_sessions[key]["starting"] = False
        return False, "❌ حداقل یک وسیله انتخاب کن."
    if "assistants_done" not in s:
        with _theft_lock:
            if key in _theft_sessions:
                _theft_sessions[key]["starting"] = False
        return False, "❌ ابتدا انتخاب دستیارها را تمام کن."
    if any(_theft_count(user_id, k) <= 0 for k in items):
        with _theft_lock:
            if key in _theft_sessions:
                _theft_sessions[key]["starting"] = False
        return False, "❌ موجودی یکی از وسایل انتخاب‌شده کافی نیست."

    # هدف به‌صورت شانسی و فقط از اعضای همین گروه انتخاب می‌شود.
    target = _theft_random_group_target(chat_id, user_id)
    if not target:
        with _theft_lock:
            if key in _theft_sessions:
                _theft_sessions[key]["starting"] = False
        return False, "❌ فعلاً عضو دیگری از این گروه که بات بتواند شناسایی کند پیدا نشد."
    if target == user_id or not _theft_member_ok(chat_id, target):
        with _theft_lock:
            if key in _theft_sessions:
                _theft_sessions[key]["starting"] = False
        return False, "❌ هدف معتبر پیدا نشد؛ دوباره تلاش کن."
    if mode == "bank" and not get_bank_account_number(target):
        # اگر این عضو حساب بانک ندارد، چند بار دیگر هدف تصادفی پیدا کن.
        for _ in range(9):
            candidate = _theft_random_group_target(chat_id, user_id)
            if candidate and candidate != user_id and get_bank_account_number(candidate):
                target = candidate
                break
        else:
            with _theft_lock:
                if key in _theft_sessions:
                    _theft_sessions[key]["starting"] = False
            return False, "❌ عضو واجد شرایط با حساب بانکی پیدا نشد."

    mx = THEFT_BANK_MAX_PERCENT if mode == "bank" else THEFT_HOUSE_MAX_PERCENT
    first, second = sorted([int(user_id), int(target)])
    with _get_user_financial_lock(first):
        with _get_user_financial_lock(second):
            if mode == "bank":
                apply_bank_interest(target)
                balance = get_bank_balance(target)
            else:
                balance = int(get_balance(target) or 0)
            max_stolen = int(balance * mx / 100)
            if max_stolen <= 0:
                with _theft_lock:
                    if key in _theft_sessions:
                        _theft_sessions[key]["starting"] = False
                return False, "❌ موجودی قابل دزدی هدف صفر است."
            stolen = random.randint(1, max_stolen)
            duration = _theft_calculate_duration(stolen, max_stolen)
            risk = max(
                THEFT_POLICE_MIN_CHANCE,
                THEFT_POLICE_BASE_CHANCE - THEFT_POLICE_REDUCTION_PER_ROLE * (len(items) + len(assistants))
            )

            # مبلغ از همین لحظه رزرو می‌شود تا هدف در طول عملیات نتواند آن را خرج کند.
            if mode == "bank":
                if not change_bank_balance(target, -stolen):
                    with _theft_lock:
                        if key in _theft_sessions:
                            _theft_sessions[key]["starting"] = False
                    return False, "❌ رزرو مبلغ دزدی انجام نشد."
            else:
                if update_diamonds(target, -stolen) is None:
                    with _theft_lock:
                        if key in _theft_sessions:
                            _theft_sessions[key]["starting"] = False
                    return False, "❌ رزرو مبلغ دزدی انجام نشد."

            if not _theft_consume_items(user_id, items):
                if mode == "bank":
                    change_bank_balance(target, stolen)
                else:
                    update_diamonds(target, stolen)
                with _theft_lock:
                    if key in _theft_sessions:
                        _theft_sessions[key]["starting"] = False
                return False, "❌ مصرف وسایل انجام نشد؛ مبلغ برگشت داده شد."

    target_user = get_user(target) or {}
    target_name = target_user.get("first_name") or target_user.get("username") or "کاربر"
    job = {
        "thief_id": int(user_id),
        "target_id": int(target),
        "target_name": str(target_name),
        "mode": mode,
        "stolen": int(stolen),
        "max_stolen": int(max_stolen),
    ...
    with _theft_lock:
        _theft_sessions.pop(key, None)
        _theft_jobs[key] = job

    timer = threading.Timer(duration * 60, _theft_finish, args=(key,))
    timer.daemon = True
    timer.start()

    return True, (
        "🚨 عملیات دزدی شروع شد!\n\n"
        "✓ | هدف توسط بات به‌صورت شانسی انتخاب شد.\n"
        "✓ | دزدی در حال انجام است...\n"
        "✓ | بعد از اتمام دزدی، بات در پیوی شما به شما اطلاع می‌دهد. ✓"
    )
...
# چون متن‌ها با کاراکترهایی مثل ◈ ، ━ ، اموجی یا عدد شروع می‌شن، الگوریتم
# bidi تلگرام گاهی جهت خط رو LTR تشخیص می‌ده و کل خط (یا بخشی از آن) به
# سمت چپ می‌ره. با اضافه‌کردن یک کاراکتر نامرئی RLM (راست‌به‌چپ) در ابتدای
# هر خط، جهت هر خط به‌صورت اجباری راست‌چین می‌شه؛ چون این کار روی تک‌تک
# صدها پیام فایل عملی نیست، به‌جاش خودِ متدهای ارسال/ویرایش پیام رو Wrap
# می‌کنیم (مشابه الگوی auto-reply بالا).
_RLM = "\u200f"
...
def _edit_message_text_rtl(text, *args, **kwargs):
    return _original_edit_message_text(_force_rtl(text), *args, **kwargs)
...
def main_menu_markup(user_id=None):
    ...
    markup.row(colored_button("🛒 فروشگاه دزدی", callback_data=f"theftstore|{user_id}"))
...
def show_or_edit_panel(chat_id, user_id, panel, text, reply_markup=None, parse_mode=None):
    """برای ورودی متنی همیشه پنل جدید بساز؛ پنل قبلی دست‌نخورده بماند.
    ادیت پنل فقط در callback handlerهای دکمه‌ها انجام می‌شود.
    """
    sent = safe_send_message(chat_id, text, reply_markup=reply_markup, parse_mode=parse_mode)
    if sent:
...
# ================== دسترسی متنی دزدی / فروشگاه ==================
@bot.message_handler(func=lambda m: text_is(m, "دزدی") and not _has_pending_step(m))
def theft_entry(message):
    if message.chat.type not in ("group", "supergroup"):
        bot.reply_to(message, "❌ بخش دزدی فقط داخل گروه فعال است.")
        return

    user_id = message.from_user.id
    wait_text = _theft_wait_message(message.chat.id, user_id)
    if wait_text:
        bot.reply_to(message, wait_text)
        return
    ...
    with _theft_lock:
        _theft_sessions[key] = {"mode": None, "items": [], "assistants": []}
    show_or_edit_panel(
        message.chat.id,
        message.from_user.id,
        "theft",
        "◈ ━━━━━ 𝗧𝗵𝗲𝗳𝘁 ━━━━━ ◈\n\n✓ | نوع دزدی را انتخاب کنید:",
        theft_main_markup(message.from_user.id),
    )


@bot.callback_query_handler(func=lambda c: c.data.startswith("theftmode|"))
def theft_mode(c):
    owner = check_panel_owner(c)
    if owner is None:
        return
    wait_text = _theft_wait_message(c.message.chat.id, owner)
    if wait_text:
        bot.answer_callback_query(c.id, wait_text.replace("\n", " "), show_alert=True)
        return
    mode = c.data.split("|")[1]
    key = (c.message.chat.id, owner)
    with _theft_lock:
        _theft_sessions[key] = {"mode": mode, "items": [], "assistants": []}
    bot.answer_callback_query(c.id)
    safe_edit_message(
        theft_session_text(*key),
        c.message.chat.id,
        c.message.message_id,
        theft_session_markup(owner, c.message.chat.id),
    )


@bot.callback_query_handler(func=lambda c: c.data.startswith("theftgear|"))
def theft_gear(c):
    owner = check_panel_owner(c)
    if owner is None:
        return
    ...
    key = (c.message.chat.id, owner)
    with _theft_lock:
        s = _theft_sessions.get(key)
    if not s or not s.get("mode"):
        bot.answer_callback_query(c.id, "ابتدا نوع دزدی را انتخاب کن.", show_alert=True)
        return
    inv = get_theft_inventory(owner).get("items") or {}
    available = [(k, n) for k, n in THEFT_ITEMS[s["mode"]] if int(inv.get(k, 0) or 0) > 0]
    m = types.InlineKeyboardMarkup()
    for k, n in available:
        tick = "✓ " if k in s.get("items", []) else ""
        m.add(colored_button(f"{tick}{n}", callback_data=f"theftselect|{k}|{owner}"))
    m.add(colored_button("✓ اتمام", callback_data=f"theftgeardone|{owner}"))
    bot.answer_callback_query(c.id)
    safe_edit_message(
        "🧰 | وسایل هایی که می‌خوای همراهت ببری رو انتخاب کن:\n\n"
        "✓ | با زدن هر وسیله، تیک کنار آن نمایش داده می‌شود.",
        c.message.chat.id,
        c.message.message_id,
        m,
    )
...
@bot.callback_query_handler(func=lambda c: c.data.startswith("theftselect|"))
def theft_select(c):
    owner = check_panel_owner(c)
    if owner is None:
        return
    ...
    if not s:
        return
    if item not in dict(THEFT_ITEMS.get(s.get("mode", "house"), [])):
        bot.answer_callback_query(c.id, "این وسیله برای این نوع دزدی نیست.", show_alert=True)
        return
    with _theft_lock:
        selected = s.setdefault("items", [])
        if item in selected:
            selected.remove(item)
            s.pop("assistants_done", None)
            msg = "وسیله حذف شد."
        elif _theft_count(owner, item) <= 0:
            msg = "❌ این وسیله را نداری؛ اول از فروشگاه تهیه کن."
        else:
...
@bot.callback_query_handler(func=lambda c: c.data.startswith("theftgeardone|"))
def theft_gear_done(c):
    owner = check_panel_owner(c)
    if owner is None:
        return
    wait_text = _theft_wait_message(c.message.chat.id, owner)
    ...
    if not s or not s.get("items"):
        bot.answer_callback_query(c.id, "حداقل یک وسیله انتخاب کن.", show_alert=True)
        return
    with _theft_lock:
        s["gear_done"] = True
    bot.answer_callback_query(c.id)
    theft_assistant_panel(c)


@bot.callback_query_handler(func=lambda c: c.data.startswith("theftassistants|"))
def theft_assistant_panel(c):
    owner = check_panel_owner(c)
    if owner is None:
        return
    wait_text = _theft_wait_message(c.message.chat.id, owner)
    ...
    with _theft_lock:
        s = _theft_sessions.get(key)
    if not s or not s.get("items"):
        bot.answer_callback_query(c.id, "ابتدا وسایل را انتخاب کن.", show_alert=True)
        return
    selected = s.get("assistants", [])
    m = types.InlineKeyboardMarkup()
    for k, n in THEFT_ASSISTANTS:
        tick = "✓ " if k in selected else ""
        m.add(colored_button(f"{tick}{n} | 10٪", callback_data=f"theftassistselect|{k}|{owner}"))
    m.add(colored_button("🚨 شروع دزدی", callback_data=f"theftstart|{owner}"))
    bot.answer_callback_query(c.id)
    safe_edit_message(
        "🧑‍🤝‍🧑 | دستیارها را انتخاب کن:\n\n"
        "📹 هکر دوربین\n"
        "🚗 راننده ماشین\n"
        "🔐 صندوق باز کن\n"
        "🔑 کلید ساز\n"
        "👜 حمال کیف\n\n"
        "✓ | سهم هر دستیار 10٪ است.",
        c.message.chat.id,
        c.message.message_id,
        m,
    )
    with _theft_lock:
        s["assistants_done"] = True


@bot.callback_query_handler(func=lambda c: c.data.startswith("theftassistselect|"))
def theft_assistant_select(c):
    owner = check_panel_owner(c)
    if owner is None:
        return
    wait_text = _theft_wait_message(c.message.chat.id, owner)
    if wait_text:
        ...
        return
    key = (c.message.chat.id, owner)
    assistant = c.data.split("|")[1]
    with _theft_lock:
        s = _theft_sessions.get(key)
    if not s:
        return
    with _theft_lock:
        selected = s.setdefault("assistants", [])
        if assistant in selected:
            selected.remove(assistant)
            msg = "دستیار حذف شد."
        else:
...
@bot.callback_query_handler(func=lambda c: c.data.startswith("theftstart|"))
def theft_start(c):
    owner = check_panel_owner(c)
    if owner is None:
        return
    ...
    if wait_text:
        bot.answer_callback_query(c.id, wait_text.replace("\n", " "), show_alert=True)
        return
    bot.answer_callback_query(c.id, "دزدی شروع شد...")
    ok, text = _theft_run(c.message.chat.id, owner)
    safe_edit_message(
        text,
        c.message.chat.id,
        c.message.message_id,
...
@bot.callback_query_handler(func=lambda c: c.data.startswith("theftrefresh|"))
def theft_refresh(c):
    owner = check_panel_owner(c)
    if owner is None:
        return
    ...
    bot.answer_callback_query(c.id)
    safe_edit_message(
        theft_session_text(c.message.chat.id, owner),
        c.message.chat.id,
        c.message.message_id,
        theft_session_markup(owner, c.message.chat.id),
    )
...
@bot.callback_query_handler(func=lambda c: c.data == "theftstore" or c.data.startswith("theftstore|"))
def theft_store(c):
    owner = check_panel_owner(c) if "|" in c.data else c.from_user.id
    if owner is None:
        return
    bot.answer_callback_query(c.id)
    safe_edit_message(
        "🛒 فروشگاه دزدی\n\n✓ | دسته مد نظر را انتخاب کنید.",
        c.message.chat.id,
        c.message.message_id,
        theft_store_markup(owner),
    )


@bot.callback_query_handler(func=lambda c: c.data.startswith("theftshop|"))
def theft_shop(c):
    owner = check_panel_owner(c)
    if owner is None:
        return
    cat = c.data.split("|")[1]
    if cat != "items":
        bot.answer_callback_query(c.id, "این بخش دیگر فعال نیست.", show_alert=True)
        return
    text = "🧰 وسایل ها\n\n✓ | وسیله موردنظر را برای خرید انتخاب کنید."
    markup = theft_store_items_markup(owner)
    bot.answer_callback_query(c.id)
    safe_edit_message(text, c.message.chat.id, c.message.message_id, markup)


@bot.callback_query_handler(func=lambda c: c.data.startswith("theftbuy|"))
def theft_buy(c):
    owner = check_panel_owner(c)
    if owner is None:
        return
    item = c.data.split("|")[1]
    price = THEFT_ITEM_PRICES.get(item)
    if not price or price <= 0:
        bot.answer_callback_query(c.id, "قیمت این وسیله هنوز تعیین نشده.", show_alert=True)
        return
    if get_balance(owner) < price:
        bot.answer_callback_query(c.id, "❌ موجودی الماس کافی نیست.", show_alert=True)
        return
    if update_diamonds(owner, -price) is None or not _theft_add_item(owner, item):
        update_diamonds(owner, price)
        bot.answer_callback_query(c.id, "❌ خرید انجام نشد؛ مبلغ برگشت داده شد.", show_alert=True)
        return
    bot.answer_callback_query(c.id, f"✅ {theft_item_name_for_buy(item)} خریداری شد؛ موجودی +1", show_alert=True)
    safe_edit_message(
        "🧰 وسایل ها\n\n✓ | وسیله موردنظر را برای خرید انتخاب کنید.",
        c.message.chat.id,
        c.message.message_id,
        theft_store_items_markup(owner),
    )


def theft_item_name_for_buy(item):
    return _theft_item_name(item)


@bot.callback_query_handler(func=lambda c: c.data.startswith("theftnoop|"))
def theft_noop(c):
    owner = check_panel_owner(c)
    if owner is not None:
        bot.answer_callback_query(c.id, "دستیار را از پنل دزدی انتخاب کن.")


@bot.message_handler(func=lambda m: text_is(m, "فروشگاه") and not _has_pending_step(m))
def theft_store_message(message):
    if not get_user(message.from_user.id):
        create_user(message.from_user.id, get_display_name(message.from_user))
    show_or_edit_panel(
        message.chat.id,
        message.from_user.id,
        "theft_store",
        "🛒 فروشگاه دزدی\n\n✓ | دسته مد نظر را انتخاب کنید.",
        theft_store_markup(message.from_user.id),
    )
...
def text_help(message):
    ...
    "🕵️ دزدی: کلمه «دزدی» را در گروه ارسال کنید.\n\n"
    "🛒 فروشگاه: کلمه «فروشگاه» را ارسال کنید.\n\n"
    "🎡 گردونه روزانه:\n"
...
def text_bank(message):
    ...
    if not user.get("bank_account_number"):
        ...
        return
    show_or_edit_panel(message.chat.id, user_id, "bank", bank_text(user_id), bank_markup(user_id), parse_mode="HTML")
...
def dynamite_timeout(message_id):
    ...
    safe_edit_message(
        f"⎋ | زمان تموم شد!\n"
        f"⎌ | {game['amount']:,} الماس به سازنده برگردونده شد.",
        chat_id=chat_id, message_id=message_id, reply_markup=None,
    )
...
def start_dynamite_flow(message, amount):
    ...
    with dynamite_lock:
        active_dynamites[sent.message_id] = {
            "chat_id": message.chat.id,
            "creator_id": user_id,
            "creator_name": creator_name,
            "opponent_id": None,
    ...
    threading.Timer(JOIN_TIMEOUT_SECONDS, dynamite_timeout, args=[sent.message_id]).start()
...
def _dynamite_cell_reveal_locked(call):
    ...
    if idx in game["revealed"] or idx == game.get("_boomed_idx"):
        bot.answer_callback_query(call.id)
        return
    bot.answer_callback_query(call.id)

    if idx == game["bomb_index"]:
        loser_id, loser_name = clicker_id, (
            game["creator_name"] if clicker_id == game["creator_id"] else game["opponent_name"]
        )
        if clicker_id == game["creator_id"]:
...
def text_spin(message):
    ...
    if won is None:
        show_or_edit_panel(message.chat.id, uid, "spin", f"⏳ گردونه آماده نیست. {remaining//3600} ساعت و {(remaining%3600)//60} دقیقه مونده.", None); return
...
def text_trophies(message):
    uid=message.from_user.id
    if not get_user(uid): bot.reply_to(message,"اول /start بزن."); return
    show_or_edit_panel(message.chat.id, uid, "trophies", trophies_text_for_user(uid), None)
...
def text_tournament(message):
    ...
    show_or_edit_panel(message.chat.id, message.from_user.id, "tournament", text, None)
...
def _send_rank(message):
    rows = _rank_rows()
    if not rows:
        show_or_edit_panel(message.chat.id, message.from_user.id, "rank", "هنوز کاربری ثبت‌نام نکرده.", None)
        return
