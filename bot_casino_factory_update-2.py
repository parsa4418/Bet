...
bot = telebot.TeleBot(BOT_TOKEN, threaded=True, num_threads=12)

# صف پردازش سبک برای کارهای پس‌زمینه‌ای که نباید پاسخ Webhook را معطل کنند.
_background_executor = ThreadPoolExecutor(max_workers=6, thread_name_prefix="bot-bg")
...
# ظرفیت انبار در هر سطح (حداکثر در سطح ۱۰ = 15,000 الماس):
...
# زمان دستگاه حفاری (ثانیه لازم برای تولید هر الماس؛ سطح ۱۰ = ۵ ثانیه)
...
def sync_factory_production(factory):
    ...
    updates = {}
    if elapsed > 0 or new_stored != int(factory.get("warehouse_stored", 0) or 0):
        updates.update({"warehouse_stored": new_stored, "last_calc_time": now})
...
def safe_edit_message(text, chat_id, message_id, reply_markup=None, parse_mode=None, retries=2):
    """ویرایش امن پیام با چند بار تلاش مجدد در صورت خطای موقتی شبکه/تلگرام.
    اگر خطا از نوع «message is not modified» باشه، یعنی محتوا از قبل همینه؛
    این خطای واقعی نیست و نباید باعث لاگ اشتباه یا شکست منطق بشه.
    """
    last_err = None
    for attempt in range(retries + 1):
        try:
            ...
            if attempt < retries:
                time.sleep(0.35)
                continue
    logging.error(f"خطا در ویرایش پیام {message_id}: {last_err}")
    return False
...
def safe_send_message(chat_id, text, reply_markup=None, parse_mode=None, reply_to_message_id=None):
    try:
        ...
        if reply_to_message_id is not None:
            # پیامی که باید بهش ریپلای می‌شد شاید پاک شده یا قابل ریپلای نیست؛
            # حداقل خود پاسخ رو (بدون ریپلای) بفرست تا کاربر بی‌جواب نمونه.
            try:
                logging.warning(f"ریپلای به پیام {reply_to_message_id} انجام نشد ({e})؛ بدون ریپلای ارسال می‌شود.")
                return bot.send_message(chat_id, text, reply_markup=reply_markup, parse_mode=parse_mode)
            except Exception as e2:
                logging.error(f"خطا در ارسال پیام: {e2}")
                return None
...
def start_bet_flow(message, amount):
    ...
    if not edited_ok:
        ...
        safe_edit_message(
            "❌ نمایش دکمه‌های شرط با مشکل مواجه شد؛ شرط لغو و مبلغ به شما برگشت داده شد.",
            chat_id=message.chat.id, message_id=sent.message_id, reply_markup=None,
        )
        return
    ...
    with dynamite_lock:
        for row in rows:
            ...
            if game["status"] == "pending":
                ...
                threading.Timer(JOIN_TIMEOUT_SECONDS, dynamite_timeout, args=[message_id]).start()
...
def factory_production_step(message, expected_user_id, panel_msg_id):
    ...
    result = bot.reply_to(
        message,
        f"✅ {amount:,} 💎 از انبار برداشت شد و به موجودیت اضافه شد."
    )
    if result:
...
def render_workers_panel(call, owner_id, factory):
    ...
    if hired > 0:
        markup.add(colored_button("➖ اخراج یک کارگر", callback_data=f"ffire|{owner_id}"))
    if level < FACTORY_MAX_LEVEL:
...
@bot.callback_query_handler(func=lambda call: call.data.startswith("ffire|"))
def factory_worker_fire(call):
    owner_id = check_panel_owner(call)
    if owner_id is None:
        return
    factory = sync_factory_production(get_factory(owner_id))
    hired = int(factory.get("workers_hired", 0) or 0)
    if hired <= 0:
        bot.answer_callback_query(call.id, "کارگری برای اخراج وجود ندارد!", show_alert=True)
        return
    factory["workers_hired"] = hired - 1
    try:
        supabase.table("factories").update({"workers_hired": hired - 1}).eq("user_id", owner_id).execute()
    except Exception as e:
        logging.error(f"خطا در اخراج کارگر: {e}")
        bot.answer_callback_query(call.id, "❌ خطا در اخراج کارگر.", show_alert=True)
        return
    bot.answer_callback_query(call.id, "✅ یک کارگر اخراج شد.")
    render_workers_panel(call, owner_id, factory)
...
def webhook():
    ...
    if request.is_json:
        ...
        if incoming_message is not None:
            # شمارش الماس گروهی نباید پاسخ هندلر اصلی را معطل کند.
            _background_executor.submit(process_group_message_for_diamond_hunt, incoming_message)
